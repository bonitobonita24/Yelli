...
- Why: PRODUCT.md declares meeting recording as a v1 feature. Recording model + RecordingStatus enum + recording-processing worker scaffold + InCallRecordingIndicator UI + meeting-controls/meeting-room scaffolding all existed from Phase 4 Parts. This sub-session closes the LiveKit Egress integration gap: server-side start/stop via livekit-server-sdk's EgressClient, RoomComposite egress writing MP4 directly to MinIO via S3Upload (forcePathStyle), webhook intake via WebhookReceiver (JWT signature verification), BullMQ worker for status transitions, and host-only UI control wiring. Result: any meeting host can start/stop a recording from the in-meeting UI; LiveKit Egress runs the recording, uploads the MP4 directly to MinIO under the tenant prefix, and fires the egress_ended webhook; our worker flips Recording.status='ready' + populates file_size_bytes + duration_seconds; the existing recordings.getDownloadUrl procedure surfaces pre-signed download URLs to authorised tenant users.
- Locked decisions (user response 2026-05-26 06:50 GMT+8):
  - Egress mode: RoomCompositeEgress (single MP4 per meeting) — simpler playback, smaller storage footprint than per-participant.
  - Storage:    MinIO via @yelli/storage S3 client (existing) — production-ready, scalable, matches existing storage pattern.
  - Permission: Host only — Meeting.host_user_id === ctx.user.id. Aligns with Zoom/Meet defaults.
- Files added:
  - apps/web/src/lib/livekit/egress-client.ts — `startRoomCompositeRecording({roomName, storageKey})` and `stopRoomEgress(egressId)`. Wraps EgressClient + EncodedFileOutput (MP4) + S3Upload (forcePathStyle). Normalises wss://→https:// for the HTTP Egress API. Explicit env validation distinguishes "Egress not configured" (503-style) from "auth failure" (401-style) for the caller.
  - apps/web/src/lib/livekit/egress-client.test.ts — 5 unit tests covering happy path (asserts EgressClient ctor args + S3Upload args + EncodedFileOutput args + normalised return shape), wss→https URL normalisation, missing API key, missing STORAGE_BUCKET, and stopRoomEgress.
  - apps/web/src/lib/livekit/webhook-verify.ts — `verifyLiveKitWebhook(rawBody, authHeader)`. Wraps WebhookReceiver.receive (JWT signature verification). Returns null on auth failure (missing/invalid header), throws ONLY when LIVEKIT_API_KEY/SECRET missing (config gap distinguishable from auth noise by caller).
  - apps/web/src/lib/livekit/webhook-verify.test.ts — 4 unit tests covering happy path, missing auth header, invalid signature (receiver throws), missing API config.
  - apps/web/src/app/api/webhooks/livekit/route.ts — non-tRPC Route Handler. Mirrors xendit pattern with WebhookReceiver swapped for x-callback-token: 503 when env-empty, 401 when WebhookReceiver returns null (logged with source IP, never the JWT), 200 + ignored when event has no egressInfo (room_started, participant_joined, etc.), 200 + enqueue on egress_started/updated/ended/failed. Deterministic jobId `livekit-egress-{event_id}` with composite fallback `egress_started:EG_xyz` when event.id is absent, so LiveKit retries between worker drains collapse at the BullMQ layer. 500 on enqueue failure so LiveKit retries per its backoff (never ack-then-fail).
  - apps/web/src/app/api/webhooks/livekit/route.test.ts — 6 unit tests: 503 env-empty, 401 unauthorized, 200 happy egress_ended with file metadata extraction (BigInt size→string, nanosecond duration→seconds), 200 ignored for non-egress events, composite jobId fallback, 500 on enqueue failure.
  - apps/web/src/lib/livekit/egress-webhook-worker.test.ts — 7 unit tests for the pure `processLiveKitEgressWebhookJob` exported from @yelli/jobs (cross-workspace import — same pattern as xendit's webhook-worker.test.ts). Covers: recording_not_found short-circuit, no_op on non-terminal events, happy egress_ended (status→ready + file_size + duration + Meeting.recording_enabled=false + AuditLog), idempotent already_ready return, egress_failed terminal failure, egress_ended carrying EGRESS_FAILED status treated as failure, idempotent already_failed return.
  - packages/jobs/src/workers/livekit-egress-webhook.ts — pure handler + BullMQ Worker wrapper following the xendit-webhook structural-type dependency-injection pattern. `LiveKitEgressWebhookPrismaClient` interface declares only the methods the handler touches (recording.findUnique/update + meeting.update + $transaction + auditLog.create). Two-layer idempotency: queue jobId + DB short-circuit when Recording already in target terminal state. Tenant context resolved by `egress_id → Recording.organization_id` lookup (LiveKitEgressWebhookJob is NOT a TenantJobBase; webhook intake has no session).
  - packages/db/prisma/migrations/20260526070000_recording_egress_id/migration.sql — ALTER TABLE recordings ADD COLUMN egress_id TEXT + CREATE UNIQUE INDEX recordings_egress_id_key + DEFAULT 0 on file_size_bytes (BigInt) + duration_seconds (Int).
  - apps/web/package.json — added `livekit-server-sdk ^2.15.3` as a dependency.
- Files modified:
  - packages/db/prisma/schema.prisma — added `egress_id String? @unique` to Recording model. Unique because every Egress lifetime maps 1:1 to one Recording. Added `@default(0)` to `file_size_bytes` (BigInt) and `duration_seconds` (Int) so a Recording row can be inserted at Egress-start time (before the file exists); the webhook worker overwrites both when egress_ended arrives with file metadata.
  - packages/jobs/src/queues.ts — added `LiveKitEgressWebhookJob` envelope interface (NOT TenantJobBase — webhook intake has no session; worker resolves org via egress_id lookup). Added `QUEUE_NAMES.livekitEgressWebhook = 'livekit-egress-webhook'` and `livekitEgressWebhookQueue` Queue instance with retry=3 (worker is idempotent so transient blips can safely re-run).
  - packages/jobs/src/index.ts — exports `livekitEgressWebhookQueue` + `LiveKitEgressWebhookJob` type + `createLiveKitEgressWebhookWorker` + `processLiveKitEgressWebhookJob` + the three handler dependency types (LiveKitEgressWebhookHandlerDeps, LiveKitEgressWebhookJobResult, LiveKitEgressWebhookPrismaClient).
  - apps/web/src/server/trpc/routers/recordings.ts — added `start` and `stop` mutations to the existing router (did NOT create a new file — keeps all recording operations co-located). `start` enforces five gates in order: (1) meeting lookup with explicit organization_id check (NOT_FOUND on tenant mismatch), (2) host check via host_user_id === ctx.user.id (FORBIDDEN), (3) active CallLog precondition via findFirst meeting_id+ended_at:null (PRECONDITION_FAILED if absent — recording cannot precede the call), (4) double-start guard via findFirst Recording status='processing' (CONFLICT), (5) startRoomCompositeRecording called BEFORE any DB write (SERVICE_UNAVAILABLE with generic client message + server-side log on LiveKit failure, no orphan row). Then a single $transaction creating the Recording row (status='processing', egress_id, storage_type='s3', recorded_by_user_id, call_log_id) + flipping Meeting.recording_enabled=true + writing CREATE AuditLog. `stop` enforces host check then best-effort stopRoomEgress (treats errors as already-stopped, never throws to client) + Meeting.recording_enabled=false + UPDATE AuditLog. Recording.status stays at 'processing' until the webhook arrives.
  - apps/web/src/components/meeting/meeting-controls.tsx — added four optional props: isRecording (default false), isRecordingActionPending (default false), onStartRecording, onStopRecording. Record/Stop button renders only when isHost AND at least one callback is provided (display-gated). Pulses red while recording, disabled while a mutation is in flight. Server-enforced via host check on recordings.start/stop.
  - apps/web/src/components/meeting/meeting-room.tsx — added `recordingLive` useState (initialised from `recordingEnabled` server prop) + `trpc.recordings.start.useMutation` + `trpc.recordings.stop.useMutation` with onSuccess flipping local state and toast feedback + onError destructive toast. Both `<InCallRecordingIndicator>` and `<MeetingControls>` read from the live state. Mutations wired only when isHost.
- Schema/migrations:
  - 20260526070000_recording_egress_id — adds egress_id + @unique + defaults. Reversible (no data loss — only adds nullable column + defaults).
- Errors encountered:
  - vi.mock factory hoisting + class-constructor mocking: arrow-function mock factories failed with "() => ({...}) is not a constructor" when production code used `new EgressClient(...)`.
  - Module-cache pollution: tests using `vi.doMock("@/env", …) + vi.resetModules()` left subsequent tests unable to import the unmocked env (`vi.doUnmock` only undoes the doMock, doesn't restore the original vi.mock value).
  - TypeScript `exactOptionalPropertyTypes: true` rejected optional props typed `() => void` when callers passed `(() => void) | undefined`.
  - Lint: import order + `import()` type annotation banned.
- Errors resolved:
  - Class-constructor mocking: switched to `vi.fn(function () { this.method = …; })` (regular function, not arrow) + `vi.hoisted()` for variables shared between top-level test code and the mock factory. Worked across EgressClient, S3Upload, EncodedFileOutput, WebhookReceiver.
  - Module-cache pollution: extracted `mockEnv(env)` helper that re-establishes ALL mocks (env + queue + verify) on every call + invoked from `beforeEach`, so every test starts from a known clean slate regardless of what prior tests did. Documented as 🔴 gotcha in lessons.md.
  - exactOptionalPropertyTypes: declared optional props as explicit `Type | undefined` unions in MeetingControlsProps + mockEnv signature.
  - Lint: moved type imports to use `import type`, fixed import order to put vitest first then named import block second per project ESLint config.



- Agent: CLAUDE_CODE (Architect + Executor: Opus 4.7 inline per [[opus-inline-escalation-pattern]] — initial estimate ~25-30K Sonnet budget, above the empirical ~20K effective ceiling per [[sonnet-effective-budget-20k]]; final actual ~70K total session context including pre-flight scans. No Sonnet dispatch this session. Ninth data point for the ~20K Sonnet budget refinement + second consecutive Batch B sub-session where pre-flight scan reduced the tier.)
- Why: PRODUCT.md L117-121 declares four v1 report types — usage summary, call detail records, revenue summary (Super Admin), department activity — exportable as CSV AND PDF. The pre-existing surface had `exportCallLogsCsv` shipped inline in `admin.ts:301-399` with a working RFC 4180 helper, a working date-range picker in `/admin/reports/page.tsx`, a working `superAdminProcedure` + `platformPrisma` + `PLATFORM:*` audit pattern in `superadmin.ts`, and an explicit "User-activity, recording-usage, and PDF-formatted reports are scheduled for a future release" placeholder on the reports page. This sub-session closes the gap by relocating the inline CSV procedure into a shared `reports` module + adding 5 new procedures (usage summary CSV/PDF + dept activity CSV/PDF + call logs PDF) mirroring it + adding a `revenue` sub-router on `superadminRouter` for the cross-tenant report + extracting two pure serializer libraries that ARE the security boundary for [[xendit-internal-id-in-api-wire]] (explicit column-list accessors that never emit fields outside the declared list). Result: all four PRODUCT.md report types ship in both CSV and PDF, every export gated by the correct procedure (adminProcedure for tenant exports, superAdminProcedure for revenue), every tenant query carries explicit organization_id defense-in-depth, every platform query writes a `PLATFORM:EXPORT_REVENUE_{CSV,PDF}` audit log, and a three-layer leak guard (Prisma select + accessor-array serializer + regression tests on bytes) defends against the xendit_*_id wire leak.
- Files added:
  - apps/web/src/lib/reports/csv.ts — pure RFC 4180 CSV serializer. `serializeCsv<TRow>(rows, columns: ReadonlyArray<CsvColumn<TRow>>)` returns string. CRLF line terminators + trailing CRLF, double-quote escaping with field-wrapping when commas/quotes/CR/LF present, ISO-8601 rendering for Date cells, empty string for null/undefined. The accessor-array design IS the security boundary: extra fields on the source row never reach output unless declared in the CsvColumn[] passed in.
  - apps/web/src/lib/reports/csv.test.ts — 9 unit tests covering RFC 4180 invariants + null/undefined empty cells + Date ISO rendering + header-only output + the xendit_*_id leak defense at the serializer layer (regression test feeds a row carrying `xendit_invoice_id: "xnd_secret_leak"` through COLUMNS that does NOT list it, asserts output does not contain the marker).
  - apps/web/src/lib/reports/pdf.ts — pure pdfkit-based PDF table generator. `generateTablePdf<TRow>({title, subtitle, orgName, columns, rows})` returns Buffer. A4 landscape, 36pt margins, header (bold title + subtitle + org name top-right) + table header with underline + auto-pagination (re-renders table header on every new page) + per-page footer (timestamp + page number). Same accessor-array security boundary as csv.ts.
  - apps/web/src/lib/reports/pdf.test.ts — 6 unit tests: PDF magic bytes `%PDF-` at byte 0, `%%EOF` trailer at end, non-empty document for empty rows array, 250-row pagination doesn't throw, null cells don't crash, xendit_*_id leak regression at the serializer layer (asserts the binary stream does not contain a `xnd_invoice_LEAK_MARKER_12345` even when fed a row with that hostile field).
  - apps/web/src/server/trpc/routers/reports.ts — six procedures: exportCallLogsCsv (relocated + refactored from admin.ts; preserves DESC ordering + 10000-row cap + `id` column for downstream backward compat), exportCallLogsPdf, exportUsageSummaryCsv, exportUsageSummaryPdf, exportDeptActivityCsv, exportDeptActivityPdf. All gated by `adminProcedure`. Shared `dateRangeInput` Zod with 366-day range cap + end>start refinement. Three query layer functions: `queryCallLogs` (CallLog findMany with org filter + range), `queryUsageSummary` (Meeting + Recording parallel findMany + JS aggregate by UTC day for meetings count + recording minutes + active hosts via Set), `queryDeptActivity` (Department + CallLog parallel findMany + JS aggregate per dept emitting zero-activity rows for every dept). PDF variants also fetch `prisma.organization.findUnique({where:{id}, select:{name:true}})` for the header — throws NOT_FOUND on stale session against a deleted org.
  - apps/web/src/server/trpc/routers/reports.test.ts — 14 unit tests: 3 explicit organization_id tests (one per query layer) + 2 RBAC FORBIDDEN tests (CSV and PDF variants) + 2 date-range validation tests (end<=start, range>366d) + 4 CSV output shape tests (CDR filename + headers + duration calc, usage aggregation correctness, dept activity zero-activity row emission, empty-data header-only) + 3 PDF output tests (filename + base64 magic bytes, org name lookup, NOT_FOUND on stale org).
  - apps/web/src/server/trpc/routers/superadmin-revenue.test.ts — 11 unit tests covering revenue sub-router: 2 RBAC tests (tenant_admin without isSuperAdmin rejected on both CSV + PDF), 3 xendit_*_id leak regression tests (Prisma select shape lacks xendit_invoice_id + nested subscription select lacks xendit_subscription_id + xendit_customer_id; CSV bytes never contain `xendit_` even with hostile mock; PDF binary never contains marker), 5 happy-path tests (aggregation by period_month + plan_tier + payment_method with two buckets including paid + failed counts and PHP amounts, CSV + PDF audit-log assertions for PLATFORM:EXPORT_REVENUE_*, cross-tenant pattern with no organization_id in where clause), 2 date-range validation tests.
  - apps/web/src/app/superadmin/revenue/page.tsx — new client page mirroring the /admin/reports pattern. State: startDate (default -90 days) + endDate (default today). Two mutations (csv + pdf) calling `trpc.superadmin.revenue.exportRevenue{Csv,Pdf}`; on success decodes content (CSV blob or base64→Uint8Array→PDF blob) + triggers browser anchor download; on error displays destructive toast. Documents [[xendit-internal-id-in-api-wire]] in a JSDoc block noting the page only renders server-shipped data — the security boundary is enforced upstream.
- Files modified:
  - apps/web/src/server/trpc/routers/admin.ts — replaced the inline 96-line reports sub-router (lines 300-395) with a comment block + an `import { reportsRouter } from "@/server/trpc/routers/reports"`. The wire path `admin.reports.exportCallLogsCsv` is preserved verbatim because adminRouter still nests `reports: reportsRouter`. Existing `admin.test.ts:198` test (which asserts org_id is in the callLog.findMany where clause via `caller.reports.exportCallLogsCsv`) still passes unchanged.
  - apps/web/src/server/trpc/routers/superadmin.ts — added `revenue: revenueRouter` to the composed superadminRouter; new `revenueRouter` exposes `exportRevenueCsv` + `exportRevenuePdf` (both `superAdminProcedure`); shared `revenueRangeInput` + `queryRevenue` (uses `platformPrisma.invoice.findMany` with EXPLICIT select clauses on both Invoice and nested subscription excluding all xendit_*_id columns per [[xendit-internal-id-in-api-wire]]); aggregates JS-side keyed by `period_month + plan_tier + payment_method`; writes audit log via `writeAuditLog(platformPrisma, {organizationId: null, action: "PLATFORM:EXPORT_REVENUE_{CSV,PDF}", entity: "Invoice", entityId: "range:start..end"})`. New imports: serializeCsv, CsvColumn, generateTablePdf, PdfColumn.
  - apps/web/src/app/admin/reports/page.tsx — full rewrite. Replaced single "Call logs (CSV)" card with three module cards (Call detail records + Usage summary + Department activity) each carrying 2 buttons (CSV + PDF). Shared date-range picker in its own Card. Added `triggerPdfDownload(filename, base64)` helper (atob → Uint8Array → Blob of type application/pdf → anchor download). Added `makeSuccessToast(label)` factory (plain function, no `use` prefix — not a hook) and `showFailureToast` plain helper. Removed the "Other reports / User-activity, recording-usage, and PDF-formatted reports are scheduled for a future release" placeholder Card.
  - apps/web/src/app/superadmin/layout.tsx — added "Revenue" link between "Organizations" and "Platform settings" in the platform nav.
- Schema/migrations: none — uses existing CallLog, Meeting, Recording, Department, Organization, Invoice, Subscription models without modification.
- Files deleted: none.
- Dependencies added:
  - pdfkit ^0.18.0 (apps/web dependencies) — pure-Node PDF generator. Server-only, no browser bundle impact. License: MIT.
  - @types/pdfkit ^0.17.6 (apps/web devDependencies).
- Errors encountered:
  - React Rules-of-Hooks violation on first draft of `/admin/reports/page.tsx`: `useExportToast(label)` factory was named with the `use` prefix and called inside `onSuccess` callbacks of 6 different `useMutation` calls. Even though the function did not internally use any React hooks, the naming + invocation pattern would trigger eslint `react-hooks/rules-of-hooks`. Caught by the PreToolUse react-best-practices auto-suggestion hook on the Write step.
  - tRPC procedure type-mismatch on first run was not encountered (typecheck passed clean on initial write).
- Errors resolved:
  - Renamed `useExportToast(label)` → `makeSuccessToast(label)` plain factory (no `use` prefix) + added separate plain `showFailureToast` helper. Inlined the toast call in each `onSuccess` callback (one factory call per mutation, deterministic). Eliminated the spread-and-override anti-pattern (spreading a "toast handler" object then overriding `onSuccess` to also trigger a download is confusing — explicit form is clearer for 6 callers).
- Architect-Execute trail (memory-governance.md §4): Single-session Opus 4.7 inline solo. Pre-flight scan (3 minutes via `mcp__plugin_context-mode_context-mode__ctx_batch_execute` covering admin/reports/page.tsx + admin.ts reports sub-router + departments.ts CSV pattern + Prisma Meeting/CallLog/Recording/Invoice/Subscription models + tRPC root + superadmin.ts current shape + lessons.md 🔴+🟤 + DECISIONS_LOG + RBAC patterns + platformPrisma usage + plan-tier decisions) reduced Tier 2 estimate to Tier 1 actual. Followed TDD: csv.test.ts written + GREEN before reports.ts existed; pdf.test.ts written + GREEN before reports.ts wired pdfkit; reports.test.ts mocked + GREEN before admin.ts inline procedure was deleted (test still passes via `caller.reports.*` routing). No subagent dispatch — coordination overhead vs Sonnet ~20K ceiling [[sonnet-effective-budget-20k]] favored inline execution. No memory-governance.md §2.5b Opus escalation logging needed (inline is the only model used).
- Two-stage review (Rule 25) PASS:
  - Stage 1 spec compliance: vs PRODUCT.md L117-121 (4 report types × 2 formats), L127-128 (tenant_admin vs super_admin scope separation), L113 (revenue by period/plan/payment exportable). All declared behaviors implemented + verified by test. ✅
  - Stage 2 quality: zero `any` types, blast radius matches the 12 touched files (10 source + 1 config + 1 package.json), TDD RED→GREEN evidenced (csv.test.ts existed and ran before csv.ts; same for pdf), no scope creep, conventional commit format planned for squash-merge. ✅
- Deployment checklist additions: none required. pdfkit is a runtime dependency added to apps/web package.json — `pnpm install` on staging/prod servers will pull it automatically via pnpm-lock.yaml. No new env vars. No new infrastructure.
- Backward compatibility: existing `trpc.admin.reports.exportCallLogsCsv` wire path preserved; CDR CSV column order preserved (id, started_at, ended_at, ..., meeting_id); CDR CSV DESC ordering + 10000-row cap preserved. Existing `admin.test.ts:198` test still passes unchanged.
- Skipped vercel-plugin auto-suggestions (per Rule 28): next-forge / next-cache-components / nextjs / react-best-practices / auth all fired during Read+Write of routers and pages. None are applicable: this is not a next-forge project (custom V31 monorepo with locked patterns), Cache Components do not apply to interactive client-component mutation pages, the auth stack is locked to Auth.js v5 per DECISIONS_LOG.md Tech Stack section. The one actionable auto-suggestion was react-best-practices catching the Rules-of-Hooks violation on the `useExportToast` naming — fixed inline (see Errors above).
...
- Why: PRODUCT.md L94-130 declares Xendit as the SaaS payment gateway with Free/Pro/Enterprise tiers, in-tree subscription state machine (active → past_due → suspended), and admin-visible billing surface. Phase 4 scaffolded the `/admin/billing` page + `billing.ts` tRPC router as stub; Item 2 (2026-05-23) shipped the plan-limit enforcement engine on top of that stub. Item 3 closes the payment-flow gap end-to-end: Xendit checkout enrichment, webhook receipt + idempotency, invoice list security hardening, grace-period suspension cron, and past-due UI banner. After Item 3, billing module flips from 🟡 scaffolded-logic-stub → ✅ depth-complete (all wire shapes present; out-of-scope: invoice PDF downloads, in-app 80%/100% usage alerts, 5-minute call duration warning — these are usage-metering concerns deferred to a future "billing polish" batch).
- Files added:
  - apps/web/src/server/lib/xendit/checkout.ts — pure helper composing Xendit invoice payload from {plan_tier, billing_email, success_redirect_url, failure_redirect_url} with PRODUCT.md-locked pricing constants; rejects free-tier checkout attempts at the type level (3a)
  - apps/web/src/server/lib/xendit/checkout.test.ts — 7 cases: happy-path Pro + Enterprise payload shape, free-tier rejection, redirect URL validation, billing_email passthrough, currency=PHP locked, duration=86400s locked (3a)
  - apps/web/src/server/lib/xendit/webhook-worker.ts — pure handler `processXenditWebhook({ event, prisma, redis, auditLogger })` handling invoice.paid (status='active'), invoice.expired (status='past_due', grace_period_end=now+7d), subscription.cancelled (status='cancelled'); two-layer idempotency (ProcessedWebhookEvent table at storage layer + Redis SETNX 24h fast-path); AuditLog every transition with PLATFORM:* prefix (3b-ii)
  - apps/web/src/server/lib/xendit/webhook-worker.test.ts — 9 cases: invoice.paid happy path / invoice.expired sets grace_period_end / subscription.cancelled / duplicate event short-circuits via ProcessedWebhookEvent / Redis fast-path / signature missing rejection / unknown event-type pass-through / organization not found / partial DB write rollback (3b-ii; extended in 3c-ii-b to assert grace_period_end=now+7d)
  - apps/web/src/app/api/webhooks/xendit/route.ts — Next.js Route Handler (POST only, no auth middleware per security.md webhook rule); x-callback-token constant-time verify via crypto.timingSafeEqual; graceful 503 degradation when XENDIT_WEBHOOK_TOKEN env var unset (dev/CI safety); enqueues to xendit-webhook BullMQ queue then returns 200 immediately (3b-i)
  - apps/web/src/app/api/webhooks/xendit/route.test.ts — 6 cases: signature verify happy path / wrong token rejection / missing token returns 503 / non-POST 405 / malformed payload 400 / enqueue called with correct payload (3b-i)
  - packages/jobs/src/workers/xendit-webhook.ts — BullMQ worker factory wrapping the pure handler; retry=3 with exponential backoff; DLQ on permanent failure (3b-ii)
  - packages/jobs/src/workers/xendit-webhook.test.ts — 6 cases: worker dispatch happy / retry on transient error / DLQ on permanent failure / job payload validation / handler error logged to AuditLog / graceful shutdown (3b-ii)
  - packages/db/prisma/migrations/20260524100300_xendit_webhook_idempotency/migration.sql — CREATE TABLE processed_webhook_events (id, provider, external_event_id, processed_at, payload_hash); UNIQUE(provider, external_event_id) (3b-ii)
  - packages/db/prisma/migrations/20260524140200_grace_period_suspension/migration.sql — ALTER TYPE subscription_status ADD VALUE 'suspended'; ALTER TABLE subscriptions ADD COLUMN grace_period_end TIMESTAMP(3) NULL (3c-ii-b)
  - packages/jobs/src/workers/grace-sweeper.ts — pure handler + Worker factory + GRACE_PERIOD_DAYS=7 constant + addDaysUtc DST-safe helper (both exported for xendit-webhook to reuse via cross-worker import — see 🔴 [[xendit-grace-sweeper-import-direction]] below); scans Subscription where status='past_due' AND grace_period_end<=now → per-row $transaction (subscription.update(status='suspended') + organization.update(subscription_status='suspended', plan_tier='free', suspended_at=now) + writeAuditLog(PLATFORM:GRACE_PERIOD_EXPIRED)); try/catch per row; uses platformPrisma to bypass L6 tenant guard per security.md cron rule #7 (3c-ii-b)
  - apps/web/src/server/lib/billing/grace-sweeper.test.ts — 5 cases: happy path single-row (all 3 writes + audit shape) / multi-row independent processing / empty result no writes / filter regression guard asserts exact WHERE clause shape including 'status: past_due' literal / partial failure tolerance with failed counter (3c-ii-b)
  - apps/web/src/components/billing/past-due-banner.tsx — client component using trpc.billing.subscription.current with retry:false; renders only when status='past_due' AND grace_period_end !== null; destructive Alert variant with formatted deadline + CTA link to /admin/billing; silent 403 degradation for non-admin sessions (3c-ii-a)
  - apps/web/src/lib/billing/past-due-banner-state.ts — pure decision logic `shouldRenderPastDueBanner(subscription)`; mirrors PlanLimitBanner extraction pattern for node-env testability; uses local SubscriptionStatus string union (Rule 13 — no @yelli/db imports in client code) (3c-ii-a)
  - apps/web/src/lib/billing/past-due-banner-state.test.ts — 5 cases: past_due+grace renders / active renders null / past_due+null renders null / suspended renders null / cancelled renders null (3c-ii-a)
- Files modified:
  - apps/web/src/server/trpc/routers/billing.ts — subscription.current select clause now exposes grace_period_end so 3c-ii-a banner can render the deadline; invoices.list query removed xendit_invoice_id from select shape per security.md AGENT PROHIBITIONS #13 (internal correlation IDs do not belong in API wire shapes — comment block added explaining the rule for future maintainers); checkout.create wired to checkout.ts helper with auth + admin-role guard (3a + 3c-i + 3c-ii-b)
  - apps/web/src/server/trpc/routers/billing.test.ts — extended subscription.current cases to assert grace_period_end is selected; new invoices.list cases asserting xendit_invoice_id is NOT in the response shape (security regression guard); checkout.create rejection paths (free-tier attempt, missing billing_email, non-admin caller) (3a + 3c-i)
  - packages/db/prisma/schema.prisma — Subscription.grace_period_end DateTime? added; SubscriptionStatus enum gained 'suspended' value; ProcessedWebhookEvent model added with @@unique([provider, external_event_id]); pnpm prisma generate run to refresh client types (3b-ii + 3c-ii-b)
  - packages/jobs/src/queues.ts — XenditWebhookJob interface + xenditWebhookQueue Queue export + QUEUE_NAMES.xenditWebhook (3b-i); GraceSweeperJob interface + graceSweeperQueue Queue export + QUEUE_NAMES.graceSweeper + registerCronJobs extended with 'grace-sweeper:cron' at '0 */6 * * *' (every 6 hours) / empty-string organizationId sentinel for cross-tenant cron per security.md cron rule #7 (3b-i + 3c-ii-b)
  - packages/jobs/src/index.ts — exports: xenditWebhookQueue, XenditWebhookJob type, createXenditWebhookWorker, processXenditWebhook (3b-ii); graceSweeperQueue, GraceSweeperJob type, createGraceSweeperWorker, processGraceSweeperJob, GRACE_PERIOD_DAYS, addDaysUtc, GraceSweeperHandlerDeps, GraceSweeperJobResult, GraceSweeperPrismaClient (3c-ii-b)
  - packages/jobs/src/workers/xendit-webhook.ts — handleInvoiceExpired now writes grace_period_end = addDaysUtc(now(), GRACE_PERIOD_DAYS) atomically with status='past_due' flip; audit after-payload includes grace_period_end ISO string; imports GRACE_PERIOD_DAYS + addDaysUtc from './grace-sweeper' (3c-ii-b)
  - apps/web/src/app/admin/billing/page.tsx — invoice list empty-state copy: "No invoices yet. Your first invoice will appear here after your initial payment." per Item 3 close-out copy review (3c-i)
  - apps/web/src/app/app/layout.tsx — mounts <PastDueBanner /> at top of shell (max-w-7xl container) above existing content; banner returns null when no past_due state so authed users with active subscriptions see no visual change (3c-ii-a)
  - apps/web/src/app/admin/layout.tsx — same PastDueBanner mount as /app layout; ensures admins see the deadline on every admin page including the billing page itself (3c-ii-a)
  - vitest.config.ts (apps/web + packages/jobs) — REDIS_URL stub added to test env so @yelli/jobs top-level Queue construction at module import time doesn't fail in node-env tests (3b-ii — see 🔴 [[yelli-jobs-module-import-side-effect]] below)
  - docs/IMPLEMENTATION_MAP.md — billing module row flipped 🟡 → ✅; Phase 8 Batch 1 Item 3 marked complete in batch organization; billing module-by-module detail section rewritten with shipped artifacts
  - .cline/memory/lessons.md — 7 🟤 decisions + 2 🔴 gotchas appended (see Errors resolved + this entry's bottom)
  - .cline/STATE.md — final rewrite: PHASE="Phase 8 Item 3 complete (pending squash-merge)", NEXT="Phase 8 Batch B (quick-wins) — confirm or restart Phase 8 batch proposal"
  - .cline/memory/agent-log.md — Item 3 close-out entry
- Files deleted: none
- Schema/migrations: 2 new migrations ship together at Item 3 close-out
  - `20260524100300_xendit_webhook_idempotency` (3b-ii — ProcessedWebhookEvent table)
  - `20260524140200_grace_period_suspension` (3c-ii-b — subscription_status enum + grace_period_end column)
  - Both require `prisma migrate deploy` on staging + production before Xendit webhook route can run live
- Errors encountered:
  - 3a: free-tier checkout payload composition initially type-narrowed to Pro|Enterprise too late — caught in TDD RED phase before any wire code; rebuilt helper to reject free-tier at the function signature (PlanTier excluding 'free' as input type) so the type system enforces the constraint at every call site
  - 3b-i: graceful 503 vs hard 500 decision — webhook handler must not crash dev/CI when XENDIT_WEBHOOK_TOKEN unset; locked the 503 degradation pattern in DECISIONS_LOG.md (env-var present + non-empty = enforce verification; missing = return 503 "Webhook receiver not configured" without enqueuing)
  - 3b-ii: importing @yelli/jobs triggered top-level Queue construction (BullMQ requires Redis connection at module load), breaking node-env vitest imports; resolved by adding REDIS_URL stub to vitest.config.ts (both apps/web and packages/jobs). Locked as 🔴 [[yelli-jobs-module-import-side-effect]]
  - 3b-ii: vitest can only run in apps/web in this repo; pure functions exported from packages/jobs/* must live in apps/web alongside related tests (decision locked — see 🟤 below)
  - 3c-i: invoices.list select clause initially included xendit_invoice_id — caught during pre-3c-i existing-surface scan; security.md AGENT PROHIBITIONS #13 violation. Removed from select; added regression test asserting xendit_invoice_id is NOT in response shape. Locked as 🔴 [[xendit-internal-id-in-api-wire]]
  - 3c-ii reorder: planned 3c-ii-a (banner) → 3c-ii-b (cron) but banner copy spec requires grace_period_end in subscription.current API which requires migration + xendit-webhook write to land first; reordered to 3c-ii-b first (data layer) → 3c-ii-a (UI banner). No semantic change — same files at same end-state — just session ordering. Locked as 🟤 decision below
  - 3c-ii-b: ESLint import/order auto-fix mangled grace-sweeper.ts imports by inserting blank newline within @yelli/db group AND complained Prisma should be type-only; manual fix to `import { type Prisma, ... }` resolved both. Locked as 🔴 [[eslint-import-order-multi-line-quirk]]
  - 3c-ii-a: SubscriptionStatus enum import from @yelli/db in client-side past-due-banner-state.ts violated Rule 13 (mobile/client code cannot import packages/db; same rule applies to all client surfaces). Fixed by mirroring the enum as a local string union with JSDoc comment documenting the sync requirement. Locked as 🟤 decision below
- Errors resolved: see above — each error caught at the earliest validation stage (typecheck or test run), no production code defects shipped. Validation across 6 commits: typecheck 0 errors / lint 0 errors / test suite 339 → 381 (+42 new cases: 7 checkout + 9 webhook-worker + 6 route + 6 BullMQ worker + 5 grace-sweeper + 5 banner-state + 4 banner-component + invoices.list extensions) / pnpm build clean / pnpm audit --audit-level=high exit 0
- Architect-Execute trail per memory-governance.md §4:
  - 3a (~22K) — Opus inline (Sonnet would have thrashed on the type-narrowing TDD cycle)
  - 3b-i (~25K) — Opus inline per §2.5b (route handler + queue scaffold + tests across 3 packages)
  - 3b-ii (~28K) — Opus inline per §2.5b (worker + idempotency + schema + 2 test files + REDIS_URL stub)
  - 3c-i (~18K) — Opus inline (security review pivoted to gap-finding after existing-surface scan revealed 80% complete; pivoted to invoices.list xendit_invoice_id removal + regression test + empty-state copy)
  - 3c-ii-b (~30K Opus actual vs ~25K estimate) — Opus inline per §2.5b (migration + schema + cron worker + xendit-webhook extension + 5 new tests + extended webhook test)
  - 3c-ii-a (~13K) — **Sonnet subagent dispatch SUCCESSFUL** — first clean Sonnet dispatch since Item 2 2a-i; banner UI atom landed correctly with TDD RED→GREEN, no thrashing. Fifth confirmed data point for ~20K Sonnet effective budget (Item 2 2a-i thrashed at ~20K, Item 3 3a/3b-i/3b-ii/3c-i/3c-ii-b all estimated ≥18K and escalated to Opus per §2.5b; 3c-ii-a at ~13K cleared comfortably). Locked as 🟤 decision below
- Cross-references: SUPERSEDES the ⬜ "Xendit Invoice/Subscription creation" + ⬜ "/api/webhooks/xendit route handler" + ⬜ "Failed-payment grace state machine" + ⬜ "Past-due UI banner" entries in IMPLEMENTATION_MAP.md Billing & Subscription section. The 3 remaining ⬜ items (invoice PDF receipts / 80%-100% usage alerts / 5-minute call duration warning) are deferred to a later "billing polish" batch — they are usage-metering concerns, not payment-flow concerns. Deployment checklist additions: XENDIT_WEBHOOK_TOKEN must be populated in .env.staging + .env.prod from CREDENTIALS.md Section 4.5 before exposing the webhook route on production traffic; 2 pending migrations ship together (see Schema/migrations).
...
- Agent: CLAUDE_CODE
- Why: Phase 8 opened with user trigger "Start Phase 8" followed by "confirmed" on the proposed Batch 1 (pre-flight hardening → plan tier enforcement → Xendit billing flow). Item 1 is governance-only housekeeping that (a) refreshes the dependency-CVE baseline now that Phase 7 #9's manual mitigation can be replaced by a real in-tree fix, (b) distills 30 🔴 gotcha entries from May 13–23 into 9 reusable guardrails (G1–G9) so future Phase 8 work defaults to the hardened pattern rather than re-discovering each trap, and (c) replaces the stale Phase-4-era "Not Yet Built" block in IMPLEMENTATION_MAP.md with a module-by-module depth gap matrix mapped to proposed Batch A/B/C routing. No source code modified — frees the next session to focus on Item 2 (plan tier enforcement engine) without inherited governance debt.
- Files added: none
- Files modified:
  - package.json — added pnpm.overrides for 8 transitive CVE patches: nodemailer @<7.0.7 → ≥7.0.7, @≤7.0.10 → ≥7.0.11 (resolves HIGH GHSA-rcmh-qjqh-p98v), @<8.0.4 → ≥8.0.4, @≤8.0.4 → ≥8.0.5; postcss @<8.5.10 → ≥8.5.10; ws @>=8.0.0 <8.20.1 → ≥8.20.1; turbo @≤2.9.13 → ≥2.9.14 (x2). The next-auth @beta.0 <beta.30 → ≥beta.30 override proposed by `pnpm audit --fix` was deliberately excluded to preserve Phase 7's stabilised JWE/session callback path — that moderate is accepted as the exploit vector (Email provider) is unreachable in Yelli's Credentials-only auth flow.
  - docs/DECISIONS_LOG.md — (a) added "SUPERSEDED 2026-05-23" banner to existing "Unfixed CVE acceptance — nodemailer GHSA-rcmh-qjqh-p98v (Phase 7 #9)" section explaining the override now resolves the CVE in-tree (`.npmrc audit-level=critical` and JSDoc mitigation kept as defense-in-depth if future @auth/core bump unwinds the override). (b) Appended new section "Phase 7 Hardening Notes — 9 Guardrails From Realtime-Engine Build" distilling 30 🔴 gotcha entries (2026-05-13 → 2026-05-23) into 9 single-sentence default-pattern guardrails: G1 webpack runtime singletons MUST use globalThis[Symbol.for], G2 runtime env gates MUST use APP_ENV not NODE_ENV, G3 dev compose MUST expose every browser-dialled port, G4 Socket.IO bootstrap MUST gate snapshots on client→server `:ready` event, G5 SocketProvider StrictMode cleanup MUST be guarded against permanent disconnect, G6 LiveKit URL MUST return NEXT_PUBLIC_LIVEKIT_URL to browser, G7 env-var renames MUST sweep all .env files + compose files atomically, G8 Prisma standalone Docker build MUST COPY engine binary explicitly, G9 PgBouncer AUTH_TYPE MUST match Postgres password_encryption. Includes domain cross-reference matrix mapping every recurring gotcha by domain (webpack/bundling, realtime/Socket.IO, LiveKit, database/Prisma, Auth.js/JWE, dependency/CVE, routing, dev environment, TypeScript strict) to its applicable guardrail.
  - docs/IMPLEMENTATION_MAP.md — "Not Yet Built" section fully rewritten. Old content (Phase 5/6/7 listed as next-to-run, plus 8 deferred-follow-up bullets) superseded by Phase-8-era module gap matrix: depth-status legend (✅ built / 🟡 scaffolded-logic-stub / ⬜ not-built / ⏸ deferred), 13-module table covering all PRODUCT.md modules with proposed Batch A/B/C routing, plus per-module gap detail for 7 🟡 modules (in-call chat / file sharing / whiteboard / recording / tenant admin analytics / billing / superadmin / reports & export) listing concrete missing capabilities with file-count and tier estimates. Reflects current main at `6f9ac04` post-Phase-7 #1-#17 + SLOT 1/2.
  - .cline/STATE.md — rewritten with PHASE="Phase 8 Batch 1 Item 1 IN PROGRESS — pending squash-merge", LAST_DONE listing all 3 governance file diffs, NEXT enumerating the 7-step close-out (CHANGELOG + agent-log + commit + squash-merge + push + new session for Item 2 + onward to Item 3), GIT_BRANCH=scaffold/phase8-item1-preflight, TIER_CLASSIFICATION=Tier 1 (3 files, ~50K context).
- Files deleted: none
- Schema/migrations: none
- Errors encountered:
  - 1 HIGH CVE flagged on initial `pnpm audit --audit-level=high` — nodemailer 6.10.1 < 7.0.11 (GHSA-rcmh-qjqh-p98v addressparser recursive-call DoS).
  - `pnpm audit --fix` auto-generated 9 overrides including a high-risk next-auth beta.25 → beta.30 jump that could regress Phase 7's JWE/session callback work.
- Errors resolved:
  - HIGH CVE resolved via pnpm.overrides "nodemailer@<=7.0.10": ">=7.0.11" — empirically safe: next-auth 5.0.0-beta.25 + @auth/core 0.41.2 + @auth/prisma-adapter 2.11.2 all accept nodemailer 7.x at runtime (peer warnings only; `pnpm install` succeeded, `pnpm --filter @yelli/web typecheck` clean, 32 test files / 299 tests pass). Re-run `pnpm audit --audit-level=high` exit 0.
  - High-risk next-auth bump avoided by manually trimming that single line from `pnpm audit --fix`'s proposed overrides before `pnpm install`; the moderate next-auth Email-misdelivery advisory is accepted with mitigation (Yelli uses Credentials provider only, Email provider not configured, exploit path unreachable).
...
- Why: STATE.md NEXT after `dc9bb3a` requested the alice→bob multi-context Playwright smoke as the visual-QA gate that would close out Phase 7 #14/#15 + #17 Rule 16 cleanup. The user picked the "same-user two-tab self-call" smoke shape (Playwright MCP is single-context — no concurrent multi-user) as the closest approximation that still exercises the IncomingCallDialog render + yellow-dot indicator code path. Outcome: backend + DB + socket layer all confirmed healthy; UI auth-gated visual-QA gate was blocked by an Auth.js `?error=Configuration` failure during programmatic login, and the smoke was downgraded to PARTIAL VERIFICATION rather than ship a workaround that wasn't worth the scope (per the user's "Document smoke as PARTIAL + move to governance update" decision).
- Files added: none
- Files modified:
  - `.cline/memory/lessons.md` — prepended NEW 🔴 [[playwright-smoke-auth-configuration-blocker]] entry at top of file documenting: (1) 6-layer findings (services healthy, Prisma seed works, Cloudflare siteverify accepts ANY string with test secret so Turnstile is provably NOT the blocker, direct POST to /api/auth/callback/credentials returns ?error=Configuration regardless of token shape, forged JWT cookie via next-auth/jwt.encode was rejected by middleware); (2) dual-Next.js-instance root cause hypothesis (stale yelli_dev_app docker container at host port 43512 alongside fresh `pnpm dev` at port 3000); (3) mitigation recipes for future smoke runs (stop the container, or export APP_PORT=43512 before pnpm dev, or add a NODE_ENV-gated `signInForTest` helper); (4) cross-references to [[trpc-client-procedure-type-missing]] and [[fresh-client-presence-snapshot-race]].
  - `.cline/STATE.md` — full rewrite for post-merge baseline (HEAD = `dc9bb3a`) + smoke PARTIAL outcome. Documents the layers that PASS (postgres + valkey + minio + livekit + livekit_egress + coturn + mailhog + pgadmin healthy; Next.js /api/health 200; socket server listening on :43515; Prisma seed creates alice + Reception dept bound to alice cleanly; bcrypt.compare replay of authorize() returns expected user object) and the layer that BLOCKS (Auth.js Configuration error on programmatic login due to dual-instance environmental confound).
  - `docs/CHANGELOG_AI.md` — this entry.
- Files deleted: none
- Schema/migrations: none (no Prisma changes; the smoke seed used upsert into existing tables — the `smoke-v31` org + alice user + Reception dept remain in dev DB as orphaned test data, idempotent for future smoke runs).
- Smoke evidence captured (2026-05-22 ~8:20pm–9:25pm GMT+8):
  - **Backing services**: `docker compose ps` shows yelli_dev_postgres (healthy), yelli_dev_valkey (healthy), yelli_dev_minio (healthy), yelli_dev_pgbouncer (up), yelli_dev_livekit + yelli_dev_livekit_egress (up), yelli_dev_coturn (up), yelli_dev_mailhog (up). pgadmin compose file referenced but container not in current ps output.
  - **App layer**: `curl http://localhost:43512/api/health` returns `{"status":"ok","service":"yelli-web"}` HTTP 200. Socket server log captured: `[socket] listening on :43515`. LiveKit signal at `http://localhost:43532/` returns 200 OK.
  - **DB seed**: `pnpm --filter @yelli/db exec tsx smoke-seed.mjs` creates Organization id `cmpgxhmlk0000jgeaa08zv1zv` (slug=smoke-v31), User id `cmpgxhmlw0002jgeal6os2ott` (email=alice-smoke-v31@yelli.local, role=tenant_admin, security_version=0+), Department id `cmpgxhmm10004jgea64gz4jbp` (name=Reception, default_user_id = alice). Replay of auth.ts authorize() flow against this user via `platformPrisma.user.findFirst(... include: organization)` + `bcrypt.compare` returns the full user object with `org_relation: true, suspended: null, bcrypt match: true`.
  - **Turnstile siteverify**: direct POST to `https://challenges.cloudflare.com/turnstile/v0/siteverify` with secret `1x0000000000000000000000000000000AA` returned `success: true` for token strings `"XXXX.DUMMY.TOKEN.XXXX"`, `"any-string"`, `"0.abc.def"`, `"0.aaaabbbbccccddddeeeeffff0123456789"` — all with `metadata: { result_with_testing_key: true }`. Empty string returned `success: false, error-codes: ["missing-input-response"]`. **Turnstile is provably NOT the auth blocker**.
  - **Programmatic login attempt**: POST to `http://localhost:43512/api/auth/callback/credentials` with valid csrfToken (from prior `/api/auth/csrf` GET) + email/password matching the seeded alice user + organizationSlug=smoke-v31 + any/no turnstileToken → response redirects to `/login?error=Configuration` (status 200 HTML body, no session cookie set). Same outcome whether redirect is followed or set to manual.
  - **Forged session JWT**: `next-auth/jwt.encode({ token: {sub, email, userId, organizationId, organizationSlug, role, isSuperAdmin, securityVersion}, secret: env.AUTH_SECRET, salt: "authjs.session-token" })` produces a 734-char JWT. Set as `document.cookie = "authjs.session-token=..."`. Navigate to /app → middleware redirects to /login, cookie cleared from browser jar. Decode side of the JWT does not accept the encode side.
- Errors encountered:
  - `?error=Configuration` from Auth.js v5 credentials POST — the Auth.js v5 docs describe this as "authorize fn threw an unhandled exception", but the direct replay of authorize() returns a clean user object without throwing. The proximate confound is the dual-instance environment.
  - Forged session JWT rejected — could be Auth.js v5 beta25 salt/encoding divergence between encode and decode paths, OR the docker container's runtime AUTH_SECRET differs from the host's .env.dev value (env_file mounts at runtime in compose, so this should be the same, but the container is 27hrs stale and may have a frozen env from an earlier .env.dev rotation).
  - Playwright MCP is single-context — true concurrent alice + bob multi-user testing impossible without spawning a second browser. User picked same-user two-tab as approximation; even that was blocked at the prior step.
- Errors resolved: 1 documented in lessons.md as a 🔴 gotcha — the [[playwright-smoke-auth-configuration-blocker]] entry includes mitigation recipes so the next smoke session does not re-walk the same 6 layers of diagnostics.
- Validation matrix (re-confirmed from `dc9bb3a` baseline — no code changes this session):
  - pnpm lint: ✓ 0 errors, 2 pre-existing warnings (unchanged from d364462 baseline)
  - pnpm typecheck: ✓ 0 errors across 8 packages
  - pnpm test: 270/270 ✓ (unchanged — no test changes, no new tests, no deletions)
  - pnpm build: ✓ Middleware 141 kB (unchanged — zero source diff this session)
  - pnpm audit --audit-level=critical: ✓ exit 0 (1 HIGH pre-documented per [[nodemailer-cve-mitigation]])
  - Smoke visual-QA gate: **PARTIAL** — backend + DB + socket layer confirmed; UI auth-gated layer deferred per 🔴 [[playwright-smoke-auth-configuration-blocker]].
- Next steps: One of three paths is recommended next session — (a) close out Phase 7 #17 Rule 16 cleanup acknowledging unit-test coverage already provides correctness validation and PARTIAL smoke is sufficient; (b) re-run smoke after `docker compose -f deploy/compose/dev/docker-compose.app.yml stop app` to remove the stale-container confound; (c) add a `NODE_ENV=development`-gated `AUTH_BYPASS_FOR_E2E` env-var test-auth helper to apps/web/src/server/auth.ts as a one-time ~20-line investment that unblocks all future Playwright smoke tests. Outstanding queue items: `(admin-bounce-prefix-symmetry)` Tier 1 cosmetic remains unchanged.
...
## 2026-05-22 — Phase 7 (fresh-client-presence-snapshot-race) — presence:ready handshake gates snapshot emission
- Agent:               CLAUDE_CODE (Opus 4.7 — direct, no Sonnet dispatch)
- Why:                 The Phase 7 #17 second-pass smoke matrix ([[rule-16-cleanup-2026-05-22]]) caught bob and carol whose own `useUserPresence.onlineSet` stayed `Set(0)` even though alice (already-logged-in observer) saw both in her set. Diagnosed as a fresh-client `presence:snapshot` race: server `attachPresenceHandlers` emitted snapshot synchronously inside `io.on("connection")` immediately after `roster.addSocket(...)`, but the client `useUserPresence` hook attaches its listener inside a `useEffect` (post-React-commit, often post-`socket.connected`). Fresh clients dropped the snapshot → empty onlineSet → cascade blocked #14 in-call yellow-dot, #15 incoming-call dialog, and #16 dept-filter (because `useEmitCallParticipation.myBoundDepartmentIds` never resolves on fresh clients → `selectIncomingCall(payload, undefined) = false` → `IncomingCallDialog` never renders).
- Files added:         none.
- Files modified:
  - `apps/web/src/server/socket/presence.ts` (+15 / -3) — added `const PRESENCE_READY_EVENT = "presence:ready"`. Replaced the synchronous `socket.emit(PRESENCE_SNAPSHOT_EVENT, ...)` inside `attachPresenceHandlers` with a `socket.on(PRESENCE_READY_EVENT, () => { if (snapshotEmitted) return; snapshotEmitted = true; socket.emit(...); })` gate. Idempotent guard defends against React StrictMode double-fire and reconnect-resume duplicate ready events.
  - `apps/web/src/server/socket/presence.test.ts` (+62 / -18) — added `fireReady(socket)` + `findSnapshotCall(socket)` helpers. Added 3 NEW RED→GREEN cases: "does NOT emit presence:snapshot on connect alone — waits for presence:ready (race fix)", "registers a presence:ready handler on the socket", "snapshot emission is idempotent on duplicate presence:ready (defensive)". Updated 2 existing cases ("emits presence:snapshot {userIds} to this socket AFTER presence:ready — including self" and "two distinct users in the same org both go online and both appear in each other's snapshot") to fire `presence:ready` between attach and assertion.
  - `apps/web/src/lib/presence/user-presence-handler.ts` (+11 / 0) — extended `MinimalSocketEventTarget` with `emit(event: "presence:ready"): unknown` overload. Added `socket.emit("presence:ready")` to `attachUserPresenceHandlers` AFTER both `socket.on(...)` calls register — the listener-attach-before-handshake-emit ordering is what the test enforces.
  - `apps/web/src/lib/presence/user-presence-handler.test.ts` (+78 / -11) — `makeFakeSocket` gained an `emit` method that captures outbound emits into an `emittedToServer` array, and the existing test-side dispatcher was renamed `fireFromServer` to disambiguate server→client delivery from client→server emits. Added 3 NEW RED→GREEN cases: "emits presence:ready to the server AFTER attaching listeners (handshake)", "emits presence:ready ONCE per attach call (a second attach is a separate handshake)", "emits presence:ready AFTER both listeners have been registered (ordering)". Renamed all destructured `emit` references in existing tests to `fireFromServer: emit` for backwards compatibility.
  - `apps/web/src/lib/socket/types.ts` (+7 / 0) — added `presence:ready: () => void` to `ClientToServerEvents` interface with a 5-line comment cross-referencing the fix and pointing at the server-side gate.
- Files deleted:       none.
- Schema/migrations:   none.
- Errors encountered/resolved:
  - None during implementation. The RED phase produced exactly the 6 expected failures (matching the 6 NEW test cases). GREEN was clean on the first implementation pass.
  - The Edit tool replace_all sweep on `const { socket, emit }` in user-presence-handler.test.ts initially over-renamed (caught all 4 occurrences including the one inside the unrelated-events test) — fixed via a follow-up Edit narrowing to the affected block. No code defect; just an editor-mechanics gotcha worth noting.
- Decisions locked: See DECISIONS_LOG.md entry "Phase 7 — fresh-client-presence-snapshot-race fix approach" (option a chosen over server-retry and tRPC-bootstrap candidates).
- Verification:
  - pnpm lint ✓ 0 errors (2 pre-existing warnings unchanged — app/layout.tsx no-css-tags + calls.test.ts non-null assertion, both outside the diff).
  - pnpm typecheck ✓ clean across all 8 packages.
  - pnpm test 261 → 267 ✓ (+6 new RED→GREEN). All 27 test files pass.
  - pnpm build ✓ 22 routes; **Middleware 141 kB UNCHANGED** (socket types are NOT on the middleware/instrumentation edge per [[instrumentation-edge-stub-required]] — types.ts is consumed by socket-server.ts which runs in the Node.js server runtime, not the Edge middleware bundle).
  - pnpm audit --audit-level=critical ✓ exit 0 (1 HIGH = pre-documented nodemailer per [[nodemailer-cve-mitigation]] + 6 moderate + 2 low all below threshold).
  - Two-stage review (Rule 25): Stage 1 spec PASS — fix matches the chosen DECISIONS_LOG option (a) verbatim, server defers snapshot to presence:ready handler, idempotent guard present, client emits after listener attach. Stage 2 quality PASS — zero `any` types added (explicit `PRESENCE_READY_EVENT` constant + typed `presence:ready: () => void` in ClientToServerEvents), TDD RED→GREEN evidenced (vitest output shows 6 RED then 6 GREEN after impl), blast radius matches the pre-declared 5-file plan exactly (+185 / -22 confirmed via `git diff --stat HEAD`), conventional commit message used.
- Dispatch retrospective: Direct Opus 4.7 single-session inline implementation (no Sonnet dispatch). Tiered Decomposition score: 5 files × 2.5 + 1 module × 5 + depth 2 × 3 = 23.5 — well below the 40 mandatory-split threshold. Session aggregate context ~55K including investigation, test/impl, validation, and governance writes. No mid-session thrashing.
- Models used:
  - planning: claude-code (Opus 4.7 — Architect classified Tier 2 single-session)
  - execution: claude-opus-4-7 direct
  - governance: gemini-2.5-flash-lite (not invoked; Opus inline)
- SKIPPED skill prompts: next-forge + nextjs (auto-suggested on Read/pnpm-build pattern matches per Vercel plugin) — task is socket.io protocol gating, not Next.js framework migration or Cache Components work. Both rejected per Rule 28 priority order (CLAUDE.md rules > plugin pack suggestions).
- Branch: `feat/fresh-client-presence-snapshot-race` (squash-merge SHA pending — captured in next session if/when merged).
- Environment state at session close: native `pnpm dev` running on port 43512 (PID owned per `ss -tlnp` — restart still needed for socket-server hot reload to pick up the new presence:ready handler; instrumentation.ts is not HMR-aware). Test fixtures from [[rule-16-cleanup-2026-05-22]] still seeded in dev DB. Next action recommended: bounce `pnpm dev` (or `bash deploy/compose/start.sh dev restart`) + re-run the multi-context Playwright smoke from #14/#15/#16 to confirm unblock end-to-end before squash-merging.

## 2026-05-22 — (fresh-client-presence-snapshot-race) SMOKE-VERIFY follow-up — uncovered + fixed SocketProvider StrictMode disconnect; #14/#15 reveal third bug (tRPC client proxy)
- Agent:               CLAUDE_CODE (Opus 4.7 — direct, no Sonnet dispatch)
- Why:                 The prior entry (above) shipped the presence:ready handshake fix but never ran an end-to-end browser smoke. This follow-up session restarted dev + ran the multi-context Playwright rig from [[rule-16-cleanup-2026-05-22]] for #14/#15/#16. Initial run showed every dept "(offline)" on every page — symptom worse than pre-fix. React fiber introspection on alice's tile found `SocketProvider.socket.connected: false` with `engineReadyState: "closed"` and `sendBuffer.length: 2` (the queued `presence:ready` emits waiting for connect). Manually calling `socket.connect()` from the probe restored the connection and Reception+Sales tiles immediately went online matching fixture bindings — proving the presence:ready fix is correct but masked by a separate `SocketProvider` lifecycle bug. Root cause: `SocketProvider`'s `useEffect` cleanup calls `socket.disconnect()`; Socket.IO does NOT auto-reconnect after explicit `.disconnect()` (its `reconnectionAttempts: 5` budget is transport-drop only). Under `reactStrictMode: true` dev double-fire (mount → cleanup → remount), the second mount attaches listeners to a permanently-dead socket. HMR + route remounts hit the same shape. Fix: extracted `ensureSocketConnected(socket)` pure helper per the [[pure-helper-extraction-pattern]] (precedent: `session-invalidation.ts`); invoked at the top of the provider effect to recover from any prior cleanup-induced disconnect.
- Files added:
  - `apps/web/src/lib/socket/ensure-connected.ts` (NEW, 26 lines) — exports `ensureSocketConnected(socket: ReconnectableSocket): void` + the minimal `{ disconnected; connect() }` interface. JSDoc cross-references the StrictMode root cause + `client.ts:54` reconnection budget.
  - `apps/web/src/lib/socket/ensure-connected.test.ts` (NEW, 65 lines) — 3 cases: connects when disconnected, no-op when connected, returns undefined. Hand-rolled fake socket per the established sibling pattern (no `vi.fn()` on the socket methods themselves — counters tracked in closure state).
- Files modified:
  - `apps/web/src/lib/socket/socket-context.tsx` (+11 / 0) — imported `ensureSocketConnected`; invoked it at the top of the existing `useEffect` BEFORE attaching `attachSessionInvalidationHandler`. The cleanup still calls `socket.disconnect()` — that path is required for real unmount on `/login` redirect from `session:invalidated` (see `session-invalidation.ts`). The guard only handles the dev StrictMode double-fire + HMR remount race; production behavior is unchanged because StrictMode does not double-fire in `next start`.
- Files deleted:       none.
- Schema/migrations:   none.
- Errors encountered/resolved:
  - RED phase: import resolved correctly; assertion shape needed to match the existing sibling-test pattern. Initial test version used `vi.fn()` directly on `socket.connect`, which produced a TypeScript intersection error (`Mock<Procedure | Constructable>` has both call and construct signatures; the `ReconnectableSocket.connect(): unknown` method-shape only accepts callable). Resolved by switching to a closure-counter fake (matches `session-invalidation.test.ts` exactly). One Edit fixed it; 0 follow-up edits needed.
- Verification:
  - pnpm lint ✓ 0 errors (2 pre-existing warnings unchanged).
  - pnpm typecheck ✓ clean across all 8 packages.
  - pnpm test 267 → 270 ✓ (+3 new RED→GREEN). All 28 test files pass.
  - End-to-end (Playwright multi-context smoke):
    - Pre-fix probe: alice (default ctx, first login) `connected: true`; bob + carol (newContext'd) `connected: false` with `sendBufferLen: 2` each — confirming the StrictMode race; the 2 buffered emits were their `presence:ready` handshakes waiting for a connect that never came.
    - Post-fix probe (fresh login, no force-connect): alice `connected: true, socketId: WCc8i8NzEIsaHGwzAAA4`; bob `connected: true, socketId: WsWjeru4uC0GigqgAAA9`; both `sendBufferLen: 0`. All Speed Dial tiles show Reception(online) + Sales(online) + Front Desk(offline) + Support(offline) — matching fixture bindings on all 3 fresh clients.
    - **#16 dept-filter ✅ UNBLOCKED** end-to-end.
    - **#14 + #15 ❌ STILL BLOCKED** by a third independent bug discovered during the same smoke: bob's `trpc.departments.myBoundDepartmentIds.useQuery()` is stuck `status: "pending", isSuccess: false, dataUpdatedAt: 0` indefinitely despite (a) the server returning `200` with the correct `["cmpfmyva2000fa8k1drezs7bw"]` (Sales dept) Sales ID, verified via direct `fetch('/api/trpc/departments.myBoundDepartmentIds?...', { credentials: 'include' })` from inside bob's evaluate context, AND (b) the dev log showing 10+ identical `GET /api/trpc/departments.myBoundDepartmentIds ... 200` responses with times 37ms–5355ms. Manual `queryClient.getQueryCache().getAll()` refetch on the matching query threw `TypeError: client[procedureType] is not a function at @trpc/client/dist/index.mjs:184`. This is a tRPC client-proxy bug (likely a missing `query`/`mutation`/`subscription` method on the dispatch path) — independent of presence + socket. Queued as `(trpc-client-procedure-type-missing)` Tier 1-2 for dedicated investigation in a separate ticket. Cascade chain: `boundDeptIds === undefined` permanently → `selectIncomingCall(payload, undefined) === false` → `IncomingCallDialog` never opens → no yellow-dot for #14 (because the in-call sync flow depends on the dialog being acted upon). Pre-existing design per `apps/web/src/lib/calls/select-incoming-call.ts` JSDoc (Phase 7 #16 decisions 3+4).
  - Two-stage review (Rule 25): Stage 1 spec PASS — `ensureSocketConnected` invoked at top of effect, behavior matches the hypothesized StrictMode race recovery, no other code paths touched. Stage 2 quality PASS — zero `any` types added, TDD RED→GREEN evidenced (3 RED then 3 GREEN), blast radius minimal (1 modified provider file + 2 new pure-helper files), conventional commit messages used.
- Decisions locked: See DECISIONS_LOG.md entry "SocketProvider StrictMode-safe reconnect via ensureSocketConnected pure helper".
- Dispatch retrospective: Direct Opus 4.7 single-session inline implementation (no Sonnet dispatch — investigative work + browser-driven smoke + governance writes are inherently sequential). Total session context approached ~135K including the prior-fix verification, the SocketProvider root-cause investigation, the helper extraction + TDD cycle, the post-fix smoke run, the third-bug diagnosis (queryClient.fetch() → tRPC procedureType TypeError), and the governance updates. Tiered Decomposition: 3 files × 2.5 + 1 module × 5 + depth 1 × 3 = 15.5 — Tier 1 lightweight. The diagnostic work is what drove context usage, not the fix itself.
- Models used:
...
- SKIPPED skill prompts: next-forge (auto-suggested on `Read` of `apps/web/**` pattern), nextjs (auto-suggested on `pnpm dev` Bash), react-best-practices (auto-suggested on `Read` of `src/components/**/*.tsx`), verification (auto-suggested on `pnpm dev`) — all four rejected per Rule 28 priority order (CLAUDE.md rules > plugin pack suggestions). The task is a Socket.IO lifecycle bug fix + a React Query diagnostic, not Next.js framework migration, Vercel deployment, React patterns advice, or build verification. Documented here so a future Governance Sync can audit the rejection rationale.
- Branch: `feat/fresh-client-presence-snapshot-race` (squash-merge SHA pending — captured below in the merge addendum after `git merge --squash` lands on main).
- Queue item created: `(trpc-client-procedure-type-missing)` Tier 1-2 — investigate why `IncomingCallDialog`'s `trpc.departments.myBoundDepartmentIds.useQuery()` produces `TypeError: client[procedureType] is not a function` from the React Query observer's queryFn while a plain `fetch('/api/trpc/...')` from the same browser context returns the correct data. Other tRPC queries in Speed Dial apparently work (the dept list renders), so the bug is procedure-shape-specific or query-vs-mutation specific. Suspect tRPC v11.17.0 + react-query v5 + Next.js dev-mode-only interaction. Unblocks #14 + #15 once fixed.
