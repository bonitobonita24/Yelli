...
- Out of scope (logged as queue items in STATE.md):
  ...
  - Browser smoke verification — Tier 1, manual — runs against the just-shipped instrumentation. Setup: start `pnpm dev`, host login + meeting create, mint guest JWT via /join/{token}, fresh Playwright MCP context (no cookies), navigate to `/app/meeting/{id}?guest=1`, observe the structured console.warn. Match the `hypothesis=` value to the corresponding fix.
