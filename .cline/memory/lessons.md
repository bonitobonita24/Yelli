...
- Files:     apps/web/src/server/trpc/routers/billing.ts (invoices.list select clause + new comment block explaining the rule), apps/web/src/server/trpc/routers/billing.test.ts (regression test asserting xendit_invoice_id is NOT in response shape)
- Concepts:  security, idor, internal-correlation-ids, api-wire-shape, prisma-select, billing, xendit, agent-prohibitions-13
- Narrative: `security.md` AGENT PROHIBITIONS #13 reads: "NEVER return tenantId in API response objects to the client. Use Prisma select or omit to exclude tenantId from all query results sent to the frontend." The same rule applies to ALL internal correlation IDs that bridge Yelli's domain model to external service IDs — `xendit_invoice_id`, `xendit_customer_id`, `xendit_subscription_id`, `xendit_payment_id`. These exist on Yelli's `Invoice` / `Customer` / `Subscription` rows for server-side reconciliation against Xendit webhook payloads. They have ZERO legitimate browser use — the frontend uses Yelli's internal `Invoice.id` (cuid) for all rendering, sorting, linking. Leaking the Xendit ID into the wire shape: (a) gives an attacker a stable handle to attempt IDOR against the Xendit dashboard if they ever harvest a session cookie, (b) leaks the payment provider identity even if the app later switches gateways, (c) creates a permanent client-API contract that future maintainers may rely on accidentally. The 3c-i existing-surface scan caught it on the FIRST grep — invoices.list was returning the raw Prisma row with no `select`, so EVERY column shipped including xendit_invoice_id. Fix: explicit `select: { id, amount, currency, status, paid_at, created_at }` clause; comment block above the select explaining the rule + cross-referencing this gotcha. Regression test asserts the response shape: `expect(result[0]).not.toHaveProperty('xendit_invoice_id')` AND `expect(Object.keys(result[0])).toEqual(['id', 'amount', 'currency', 'status', 'paid_at', 'created_at'])`. **Operational rule for future billing/integration routers**: any tRPC select clause on a Prisma model that has columns named `{provider}_{anything}_id` MUST explicitly enumerate the columns it returns — NEVER rely on default-select-everything. Use `omit` if more convenient (`omit: { xendit_invoice_id: true, xendit_customer_id: true }`) but `select` is preferred for type narrowing. Audit-grep rule for code review: `grep -rn "xendit_.*_id\|stripe_.*_id\|paypal_.*_id" apps/web/src/server/trpc/routers/` — every hit must be inside a `where` clause (correlating internal record to external ID for server-side reconciliation) or inside an `update`/`create` payload — NEVER inside a `select` or response shape. Cross-references: [[l6-super-admin-bypass-leak]] (sibling — defense-in-depth pattern for tenant scoping), security.md AGENT PROHIBITIONS #13 (the parent rule).

## 2026-05-25 — 🔴 [[xendit-grace-sweeper-import-direction]] When two BullMQ workers need to share a constant or helper, the canonical definition lives in the worker that OWNS the domain semantically — not in the chronologically older worker
- Type:      🔴 gotcha
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3c-ii-b — locked at Item 3 close-out
- Files:     packages/jobs/src/workers/grace-sweeper.ts (canonical GRACE_PERIOD_DAYS + addDaysUtc; both exported), packages/jobs/src/workers/xendit-webhook.ts (imports both from './grace-sweeper' to compute grace_period_end on invoice.expired)
- Concepts:  bullmq, workers, cross-worker-imports, domain-ownership, import-direction, grace-period, xendit, code-organization
- Narrative: 3c-ii-b needed to set `grace_period_end = now + 7 days` in two places: (1) xendit-webhook handleInvoiceExpired (the moment the past_due transition fires); (2) grace-sweeper cron worker (which reads the column but does not write it — it only transitions past_due → suspended when the deadline passes). The 7-day constant + DST-safe `addDaysUtc(date, days)` helper must be a single source of truth — if they ever diverge (webhook writes 7d, sweeper checks against 8d), past_due rows would either sweep too early or never sweep at all. The instinct is to put the canonical definition in xendit-webhook.ts because it's the older / more central worker — shipped in 3b-ii, lives in the busiest part of the payment flow, has more callers across the codebase. WRONG. The grace-sweeper OWNS the grace-period domain semantically — it is the worker whose entire purpose is the grace-period state machine; xendit-webhook only INITIALIZES the value as part of its much broader payment-event handling. Domain ownership beats chronology for import direction. Fix: canonical `GRACE_PERIOD_DAYS = 7` and `addDaysUtc(date, days)` live in grace-sweeper.ts (both exported); xendit-webhook.ts imports both via `import { GRACE_PERIOD_DAYS, addDaysUtc } from './grace-sweeper'`. **Operational rule for future cross-worker shared constants**: when constant or helper X is used in worker A AND worker B, ask "which worker is X about?" — that worker owns the export. If X is about scheduling/timing (CRON_INTERVAL, RETRY_BACKOFF), it lives in the worker whose schedule is being defined. If X is about a state machine (the constant that controls a transition), it lives in the worker that owns the transition logic, not the worker that observes/triggers it. **ESLint quirk caught the same day**: the codebase's ESLint config has an `import/order` auto-fix bug — when the import to be fixed is on a multi-line destructured import from @yelli/db, the auto-fix inserts a blank newline within the import group, then complains that Prisma should be type-only on the next round. Manual fix: convert to mixed-mode syntax `import { type Prisma, prisma } from "@yelli/db"` (single `type` modifier on the Prisma type, no blank lines). Locked separately as 🔴 [[eslint-import-order-multi-line-quirk]] below. Cross-references: [[yelli-jobs-module-import-side-effect]] (sibling — module-level Queue construction side effect), [[xendit-internal-id-in-api-wire]] (sibling — billing security rule from same Item 3 close-out).

## 2026-05-25 — 🔴 [[yelli-jobs-module-import-side-effect]] Importing @yelli/jobs triggers top-level BullMQ Queue construction — REDIS_URL must be set even in node-env test environments
- Type:      🔴 gotcha
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3b-ii — locked at Item 3 close-out
- Files:     apps/web/vitest.config.ts (REDIS_URL stub), packages/jobs/vitest.config.ts (REDIS_URL stub), packages/jobs/src/queues.ts (the top-level Queue construction sites)
- Concepts:  bullmq, module-import-side-effects, vitest, node-env-tests, redis-url, top-level-construction, env-stub
- Narrative: `packages/jobs/src/queues.ts` constructs BullMQ `Queue` instances at module scope (top-level — runs at import time, not at function-call time): `export const xenditWebhookQueue = new Queue(QUEUE_NAMES.xenditWebhook, { connection: ... })`. BullMQ requires a Redis connection at Queue construction. If REDIS_URL is unset or invalid at import time, the import throws — even if the importing code never enqueues anything. Discovered during 3b-ii when authoring `webhook-worker.test.ts` (a pure-function test of the handler, NO Queue or BullMQ usage — just tests the handler's database side effects given a fake event). The test imports from `apps/web/src/server/lib/xendit/webhook-worker.ts`, which imports `XenditWebhookJob` type from `@yelli/jobs`, which transitively executes `queues.ts` at module load → BullMQ tries to connect to Redis → undefined REDIS_URL → Zod validation throws → test fails to even start. Fix: REDIS_URL stub added to both vitest.config.ts files (apps/web + packages/jobs) under `test.env`: `REDIS_URL: 'redis://localhost:6379'`. The stub is a valid URL shape (passes Zod `.url()`); BullMQ never actually connects because the test never calls `.add()` on the Queue — only the Queue _exists_, which only requires a valid URL string at construction (Redis connection is lazy on first enqueue). **Operational rule for future test files that touch @yelli/jobs (directly or transitively)**: any vitest config in this repo that imports anything that transitively pulls @yelli/jobs MUST stub REDIS_URL. The same applies to any future BullMQ Queue export — keep Queue construction at module scope (it's the BullMQ idiomatic pattern) and stub REDIS_URL in test envs rather than refactoring to lazy-init. **Companion architectural rule** (also locked from 3b-ii): vitest can only run in apps/web in this repo's current setup; pure functions exported from packages/jobs/* should have their tests live in apps/web alongside related domain tests (see grace-sweeper.test.ts in apps/web/src/server/lib/billing/). This is the 🟤 [[vitest-in-apps-web-only]] decision documented below. Cross-references: [[xendit-grace-sweeper-import-direction]] (sibling — cross-worker import direction), [[vitest-in-apps-web-only]] (the companion decision on where tests live).

## 2026-05-25 — 🔴 [[eslint-import-order-multi-line-quirk]] ESLint import/order auto-fix mangles multi-line destructured @yelli/db imports — manual mixed-mode syntax required
- Type:      🔴 gotcha
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3c-ii-b — locked at Item 3 close-out
- Files:     packages/jobs/src/workers/grace-sweeper.ts (the trigger case — multi-line destructured import from @yelli/db including Prisma type + prisma client + writeAuditLog helper)
- Concepts:  eslint, import-order, type-imports, prisma, monorepo, consistent-type-imports, auto-fix-bug
- Narrative: The codebase's ESLint config has a quirk in how `import/order` and `@typescript-eslint/consistent-type-imports` interact when the import in question is multi-line. Reproducer: grace-sweeper.ts imports `{ type Prisma, prisma, writeAuditLog }` from `@yelli/db` (three symbols, one of which is a TS type). When ESLint's auto-fix runs on import-order violations elsewhere in the file, it (a) shuffles the imports correctly per group order but (b) for the multi-line destructured @yelli/db import specifically, it inserts a blank newline within the import group — splitting the @yelli/db imports across two blocks. Then `@typescript-eslint/consistent-type-imports` complains on the next lint pass that Prisma should be type-only (`import type { Prisma }`). Auto-fix again: that creates a SEPARATE `import type { Prisma }` line, then `import/order` complains the type-only import is in the wrong position. The two rules pingpong. Manual fix: collapse to single mixed-mode import `import { type Prisma, prisma, writeAuditLog } from "@yelli/db"` — the inline `type` modifier on just `Prisma` satisfies consistent-type-imports without producing a separate `import type` line; the single import line removes the multi-line trigger for import/order's auto-fix bug. **Operational rule**: when adding a multi-symbol destructured import from @yelli/db that includes at least one type, prefer mixed-mode syntax `import { type X, y, z }` over separate `import type { X }` + `import { y, z }` lines. Audit-grep for the pattern that triggers the bug: `grep -rn "^import {" apps/ packages/ -A 5 | grep -B1 "from .@yelli/db" | head -50` — look for any multi-line imports from @yelli/db; if found, verify they use mixed-mode `{ type X, y }` syntax. Cross-references: [[xendit-grace-sweeper-import-direction]] (sibling — caught during the same session).

## 2026-05-25 — 🟤 [[sonnet-effective-budget-20k]] Sonnet 4.6 subagent effective budget is ~20K tokens — NOT memory-governance.md §1's theoretical 30K
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Item 3 close-out — locks the ~20K refinement across Items 2 + 3
- Files:     .cline/memory/lessons.md (this entry), proposed amendment to .claude/rules/memory-governance.md §1 Step 2.5 (deferred — apply when next editing memory-governance.md)
- Concepts:  memory-governance, architect-execute, sonnet-budget, opus-escalation, thrashing-threshold, task-decomposition
- **Why**: memory-governance.md §1 Step 2.5 sets the Sonnet subagent task budget at ≤30K estimated tokens. Empirical data across Phase 8 Items 2 + 3 (five sub-sessions of confirmed thrash or successful pass) shows the real threshold is ~20K. Item 2 2a-i estimated at ~22K and Sonnet thrashed (autocompact 3× in 3 turns) — files were authored correctly but the validation phase ran out of context. Item 3 3a/3b-i/3b-ii/3c-i/3c-ii-b were ALL estimated ≥18K and escalated to Opus inline per §2.5b on the principle "if we expect thrash, don't dispatch — Opus inline is cheaper than Sonnet thrash + Opus salvage." Item 3 3c-ii-a was estimated at ~13K and Sonnet dispatched cleanly with no thrash — first clean Sonnet dispatch since Item 2 2a-i. Five data points (one thrash + four successful Opus escalations + one clean Sonnet at ~13K) converge on ~20K as the real effective Sonnet ceiling for this codebase + this task style (TDD RED → GREEN → REFACTOR + lint + typecheck + test in a single session).
- **How to apply**: any task estimated using the §1 Step 2.5 formula:
  - **≤15K**: dispatch to Sonnet (clean pass expected)
  - **15-18K**: dispatch to Sonnet but flag in handoff prompt as "near budget — re-estimate after first 2 tool calls, request a checkpoint if context creep is observed"
  - **≥18K**: escalate to Opus inline per §2.5b — do NOT dispatch Sonnet, even if §1 says 30K is the ceiling. The 30K theoretical includes pure model context window; the practical budget after system prompt + tool definitions + reasoning overhead + tool result accumulation is ~20K
  - **OR split below 15K**: any task that splits cleanly into atoms of ≤15K each is preferable to one ≥18K Sonnet task. Coordination overhead of 2 small Sonnets < cost of 1 thrashed Sonnet + Opus salvage. Coordination overhead is ~25% of task work; tasks ≤15K can absorb that and still net out faster than ≥18K Sonnet thrash.
- **Proposed memory-governance.md §1 Step 2.5 amendment** (apply on next governance edit): replace "Sonnet 4.6 has a 60K token context window. After accounting for system prompt, tool definitions, and reasoning overhead, the safe execution budget is 30K tokens per subagent task" with "Sonnet 4.6 has a 60K token context window. EMPIRICAL safe execution budget (observed across Phase 8 Items 2 + 3, 5 data points) is ~20K tokens per subagent task. Tasks estimated 15-18K should dispatch with a checkpoint instruction; tasks ≥18K should escalate to Opus inline per §2.5b OR split below 15K. The original 30K figure is a theoretical ceiling that does not account for tool result accumulation across TDD + lint + typecheck + test cycles."
- Cross-references: [[opus-inline-escalation-pattern]] (companion below — when to skip Sonnet entirely), memory-governance.md §1 Step 2.5 (the rule being refined), memory-governance.md §2.5b (the Opus escalation path).

## 2026-05-25 — 🟤 [[opus-inline-escalation-pattern]] When Sonnet thrash is anticipated, Opus inline is cheaper than Sonnet dispatch + Opus salvage
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Items 2 + 3 — pattern locked at Item 3 close-out
- Files:     none (process decision)
- Concepts:  memory-governance, architect-execute, opus-escalation, cost-comparison, sonnet-thrash-recovery
- **Why**: Cost analysis from Item 2 2a-i shows that when Sonnet thrashes mid-task, the recovery cost is: (a) the original Sonnet tokens consumed before thrash (typically ~50K accumulated tool results + reasoning), (b) the Opus tokens needed to salvage — re-read Sonnet's output, validate it, run remaining tests/lint/typecheck, fix any issues, commit. Opus salvage typically costs ~80-100K. Total: ~150K. Compare to Opus inline from the start: ~100K. **Opus inline is ~33% cheaper than Sonnet thrash + Opus salvage** AND avoids the risk of incomplete or incorrect Sonnet output going unnoticed.
- **How to apply**: if the §1 Step 2.5 token estimate is ≥18K (per [[sonnet-effective-budget-20k]]), DO NOT dispatch Sonnet — go directly to Opus inline. This is not "give up on Sonnet" — it is "respect the empirical budget." Sonnet remains the correct choice for atomic ≤15K tasks where the work is mechanical (single file edit, single config update, single test add). The Architect-Execute model is preserved; Opus inline counts as "Executor: Opus per §2.5b" in the CHANGELOG_AI Architect-Execute trail. Document each Opus inline dispatch in STATE.md with the §1 Step 2.5 token estimate that triggered the escalation.
- **When NOT to apply** (counter-examples): if the task is truly mechanical (rename a variable across 5 files, regenerate a snapshot, add a single field to a Zod schema), Sonnet is still correct even at 18-20K. The thrash trigger is the COMBINATION of large context + TDD cycle + multiple validation runs (lint + typecheck + test + build). A pure refactor at 20K usually clears.
- Cross-references: [[sonnet-effective-budget-20k]] (the budget threshold this decision applies above), memory-governance.md §2.5b (the escalation path being formalized).

## 2026-05-25 — 🟤 [[existing-surface-scan-before-scaffold]] Always check existing surfaces before scaffolding from STATE.md scope spec
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3c-i — locked at Item 3 close-out
- Files:     none (process decision)
- Concepts:  state-md, scope-spec, existing-surface-scan, gap-finding, tier-classification, pivoting
- **Why**: STATE.md scope specs are written for the worst case — they assume start-from-zero. 3c-i was scoped in STATE.md as "build invoice list with security hardening + 5-7 new files." A 3-minute existing-surface scan (`ls apps/web/src/app/admin/billing/ && grep -rn "invoices" apps/web/src/server/trpc/routers/billing.ts`) revealed that `/admin/billing/page.tsx` already rendered an invoice list, and `billing.ts` already had an `invoices.list` query — but the select clause leaked `xendit_invoice_id` (caught as 🔴 [[xendit-internal-id-in-api-wire]]) and the empty-state copy was missing. The session pivoted from "build new" (Tier 3, ~25K) to "gap-finding" (Tier 1, ~18K) — 1 security fix + 1 copy update + 2 regression tests = ~10 lines of source code shipped instead of 5-7 new files. Without the scan, 3c-i would have either duplicated the existing invoices.list as a new procedure (causing a regression — two procedures returning different shapes) or rebuilt the existing page (causing a merge conflict + lost work).
- **How to apply**: at every Phase 7 / Phase 8 session start, BEFORE TaskCreate, run:
  1. `grep -rn "<target-feature-name>" apps/web/src/server/trpc/routers/` — does the router exist?
  2. `ls apps/web/src/app/<target-route>/` — does the page exist?
  3. If ANY existing surface found → read it before scoping. The session pivots to gap-finding (security review, missing tests, copy polish) rather than new construction.
- **What this is not**: this rule does NOT replace STATE.md as the planning artifact. STATE.md scope specs remain the source of truth for the END-STATE the session must achieve. The existing-surface scan only adjusts the PATH (build new vs gap-find) to that end-state. If the scan reveals the existing surface is non-conformant, the session still has to bring it into conformance — that work is just narrower than "build from zero."
- Cross-references: [[xendit-internal-id-in-api-wire]] (the security gap caught by 3c-i's existing-surface scan), Rule 24 (fresh context per Part / Phase — STATE.md is read FIRST).

## 2026-05-25 — 🟤 [[grace-period-init-on-webhook-not-cron]] grace_period_end is set by the WEBHOOK on invoice.expired, never by the cron
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3c-ii-b — locked at Item 3 close-out
- Files:     packages/jobs/src/workers/xendit-webhook.ts (writes grace_period_end on invoice.expired), packages/jobs/src/workers/grace-sweeper.ts (only TRANSITIONS past_due → suspended; never initializes grace_period_end)
- Concepts:  grace-period, state-machine, xendit-webhook, cron-worker, ui-deadline-availability
- **Why**: The grace period state machine has two distinct events: (a) ENTER past_due (invoice.expired fires; subscription transitions from active to past_due) and (b) EXIT past_due to suspended (the 7-day deadline passes; cron transitions past_due to suspended). The two events have different timing semantics. Event (a) is webhook-driven — it fires the moment Xendit declares the invoice expired (typically immediately after the grace window Xendit itself observes before declaring expiration). Event (b) is time-driven — it fires when the cron next runs after `now > grace_period_end`. The UI banner (`PastDueBanner`) needs the deadline available the moment the past_due transition fires, NOT on the next cron tick (up to 6 hours later given the `0 */6 * * *` schedule). If the cron set grace_period_end, the banner would be invisible for up to 6 hours after past_due → bad UX.
- **How to apply**: any future state machine where (a) a webhook drives the ENTER transition and (b) a cron drives the EXIT transition, and (c) the UI needs to render the deadline between (a) and (b), MUST initialize the deadline timestamp in the webhook, not the cron. The cron OBSERVES the deadline (filter shape: `where status='X' AND deadline_field <= now`); it does not write the deadline. Test: assert the webhook handler writes the deadline atomically with the status flip; assert the cron filter shape matches the writing semantics (the regression test for the cron should verify the WHERE clause includes both the status literal AND the deadline comparison — see grace-sweeper.test.ts's "filter regression guard" case).
- **Related**: the cron's idempotency contract IS the filter shape itself — `status='past_due' AND grace_period_end<=now` excludes already-suspended rows by construction, so re-runs are safe without needing a separate ProcessedJobEvent-style table. Locked in [[cron-idempotency-via-filter-shape]] below.
- Cross-references: [[cron-idempotency-via-filter-shape]] (sibling — the cron's own idempotency guarantee), [[xendit-grace-sweeper-import-direction]] (sibling — where the shared constants live), security.md cron rule #7 (cross-tenant cron pattern).

## 2026-05-25 — 🟤 [[cron-idempotency-via-filter-shape]] Cron idempotency via filter shape, not via ProcessedJobEvent table
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3c-ii-b — locked at Item 3 close-out
- Files:     packages/jobs/src/workers/grace-sweeper.ts (filter shape implements idempotency), apps/web/src/server/lib/billing/grace-sweeper.test.ts (filter regression guard case)
- Concepts:  cron, idempotency, bullmq, filter-shape, exclude-by-construction, regression-guard
- **Why**: Xendit webhooks use a ProcessedWebhookEvent table (@@unique(provider, external_event_id)) for idempotency because webhook deliveries can repeat arbitrary events with the same external_event_id and there is no domain-state way to tell a duplicate from a legitimate retry. Crons are different — a cron with filter shape `where status='past_due' AND grace_period_end<=now` excludes any row that has already been transitioned to suspended (the filter no longer matches them). Re-running the cron is therefore safe by construction, without needing a separate ProcessedJobEvent table tracking "which subscriptions did I sweep last cycle?" The filter shape IS the idempotency guarantee.
- **How to apply**: any future cron with a transition operation (read rows matching condition X → mutate them to no longer match condition X) is naturally idempotent. Add a regression test that asserts the EXACT WHERE clause shape — if a future refactor widens the filter to include 'cancelled' rows or drops the `status='past_due'` literal, the test fails. The regression test should be more specific than "cron processed N rows" — it should pin down the exact filter expression. See grace-sweeper.test.ts's "filter regression guard" case for the pattern: `expect(prisma.subscription.findMany).toHaveBeenCalledWith({ where: { status: 'past_due', grace_period_end: { lte: expect.any(Date) } } })`.
- **When NOT to apply** (counter-examples): if the cron's mutation doesn't change the filter-matching state (e.g., a cron that sends reminder emails for past_due subscriptions but doesn't mutate the row), the filter is NOT a sufficient idempotency guarantee — you need a separate "last_reminder_sent_at" column or a ProcessedJobEvent table.
- Cross-references: [[grace-period-init-on-webhook-not-cron]] (sibling — the state machine's other half), security.md cron rule #7.

## 2026-05-25 — 🟤 [[3c-ii-reorder-data-before-ui]] When UI copy depends on a data field, ship the data layer first even if the original plan ordered UI first
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3c-ii — reorder decision locked at Item 3 close-out
- Files:     none (process decision)
- Concepts:  task-decomposition, data-ui-ordering, dependency-discovery, mid-plan-reorder, state-md-checkpoint
- **Why**: Item 3 was originally decomposed as 3a (Xendit checkout) → 3b (webhooks+queue) → 3c (UI + grace state machine). At 3c, the plan further split as 3c-i (invoice list) → 3c-ii-a (banner UI) → 3c-ii-b (grace cron). When 3c-ii-a started, the banner copy spec required `subscription.grace_period_end` to be available on the wire — but `grace_period_end` did not exist yet (it's added by 3c-ii-b's migration). So 3c-ii-a was blocked on 3c-ii-b. Reordered to 3c-ii-b (data layer first) → 3c-ii-a (UI consuming the new wire shape). End-state files were identical to the original plan — only session ordering changed.
- **How to apply**: at every Phase 8 sub-session start (after reading STATE.md), verify the data dependencies for the planned work. If the UI depends on a field/column/wire-shape that isn't in the codebase yet, swap the order — data layer first, UI after. Document the reorder in STATE.md as the next checkpoint so future readers know why session N+1 was different from the original plan. The framework-level rule: **end-state equivalence is invariant, but session ordering is mutable to respect data dependencies.**
- **Generalization**: same pattern applies to any "UI before data" plan ordering. The instinct to ship UI first ("see something on screen!") is wrong when the UI's behavior depends on a not-yet-existent data shape — you'd ship a banner that renders nothing (the data is undefined) and call it shipped, then ship the data layer and discover the banner had a wrong condition. Better: ship data, then ship UI consuming the data, both verified against real wire shapes.
- Cross-references: [[grace-period-init-on-webhook-not-cron]] (sibling — the data layer that 3c-ii-b shipped), Rule 24 (fresh context per Part — STATE.md captures the reorder).

## 2026-05-25 — 🟤 [[vitest-in-apps-web-only]] Pure functions exported from packages/jobs/* must have their tests in apps/web alongside related domain tests
- Type:      🟤 decision
- Phase:     Phase 8 Batch 1 Item 3 sub-session 3b-ii — locked at Item 3 close-out
- Files:     apps/web/src/server/lib/billing/grace-sweeper.test.ts (tests grace-sweeper.ts which lives in packages/jobs/src/workers/), apps/web/src/server/lib/xendit/webhook-worker.test.ts (tests webhook-worker logic), apps/web/vitest.config.ts
- Concepts:  vitest, monorepo, test-location, packages-jobs, pure-functions, redis-url-stub
- **Why**: This repo's vitest setup currently only fully works in `apps/web` (vitest.config.ts, vitest-setup, mocks all live there). Setting up a separate vitest workspace in `packages/jobs` would require: (a) duplicating mock setup, (b) maintaining two REDIS_URL stubs, (c) handling the @yelli/db client mock in two places, (d) running `pnpm --filter @yelli/jobs test` separately from `pnpm --filter @yelli/web test` (which the CI doesn't currently do). The cost-benefit is clear: just put the tests in apps/web alongside the related domain tests (grace-sweeper.test.ts → apps/web/src/server/lib/billing/; webhook-worker.test.ts → apps/web/src/server/lib/xendit/). The pure-function imports work fine across the package boundary — `import { processGraceSweeperJob } from '@yelli/jobs'`.
- **How to apply**: any new pure function exported from packages/jobs/* (or packages/storage/*, or any package that doesn't have its own vitest setup) should have its test file in apps/web at a path that matches the function's domain — billing functions in apps/web/src/server/lib/billing/, storage functions in apps/web/src/server/lib/storage/, etc. The file naming convention: `<function-name>.test.ts` alongside any related apps/web wrapper. Verify the test still imports the function from the package (`from '@yelli/jobs'`), not from a relative path — this catches breaking changes to the package's public exports.
- **When this changes**: if a future session sets up a dedicated vitest workspace in packages/jobs (e.g. because the package grows past ~10 test files and the apps/web test directory becomes cluttered), this decision should be revisited. The trigger threshold is ~10 test files; current state is 2 (xendit-webhook + grace-sweeper). Not urgent.
- Cross-references: [[yelli-jobs-module-import-side-effect]] (sibling — REDIS_URL stub requirement that makes this test-location decision feasible).

## 2026-05-23 — 🔴 [[webpack-module-duplication-singleton-trap]] Webpack splits a module into multiple compiled chunks when one chunk uses `await import()` and another uses static `import` — module-local singletons silently bifurcate across the duplicates
- Type:      🔴 gotcha
- Phase:     Phase 7 follow-up SLOT 2 — closed the incoming-call-dialog-not-rendered ticket from the 2026-05-23 PM handoff
- Files:     apps/web/src/server/socket/server.ts (hoisted `ioInstance` from module-local `let` to `globalThis[Symbol.for("yelli.socket.io")]`), apps/web/src/server/socket/server.test.ts (NEW — 5 RED→GREEN cases including the duplication scenario simulated via vi.resetModules())
- Concepts:  webpack, module-duplication, dynamic-import, tree-shaking, singleton, globalThis, symbol-for, socket-io, next-instrumentation, route-chunk, instrumentation-chunk
- Narrative: Empirical Playwright smoke for SLOT 2 proved H3 from the STATE.md handoff list: alice's tRPC `calls.initiate` returned successfully (alice navigated to `/app/call/{callId}`), bob's socket was healthy (presence frames flowed), bob's `myBoundDepartmentIds` tRPC returned the correct Sales dept id — yet bob received ZERO `call:incoming` WebSocket frames in the 3.5s window after alice's click. Bundle forensics on `apps/web/.next/server/chunks/` revealed the structural cause: Next.js 15's instrumentation.ts uses `await import("@/server/socket/server")` (dynamic import → emitted into chunk 7947, module id 67947, exposes `createSocketServer:()=>l`); tRPC `calls.ts` uses static `import { getIO }` from the same path (emitted into chunk 9865, exposes `H:()=>d` for getIO, NO reference to module 67947). Webpack tree-shook the route chunk down to only `getIO` — that copy's `let ioInstance = null` was thus NEVER assigned, because the chunk that runs `createSocketServer` (which assigns it) is a different module instance entirely. Result: every `getIO()` call from any tRPC router or Route Handler returned null, the `if (io !== null)` guard at `calls.ts:59` silently no-op'd the emit, and `emitToOrg(io, orgId, "call:incoming", ...)` was effectively dead code in containerized production builds for the entire lifetime of the new auth-gated socket server (since Phase 7 #15 — memory 2424, 2026-05-19). Presence + in-call rosters appeared to work because the socket clients DID join their org channels (via `attachPresenceHandlers` + `attachInCallHandlers` running inside `io.on("connection")` in the instrumentation-chunk copy — both run server-side via the live io); but ANY emit ORIGINATING from a tRPC route landed on a dead null io. The audit reach is therefore wider than just call:incoming: any future tRPC endpoint that calls emitToOrg via getIO is silently broken under the same trap. Fix: hoist the singleton to `globalThis[Symbol.for("yelli.socket.io")]`. `Symbol.for(...)` returns the same symbol value across module copies (same realm) so both chunks read/write the same slot. The TDD test (`server.test.ts`) simulates the duplication via `vi.resetModules()` then verifies `copyB.getIO !== copyA.getIO` (proves distinct module instances) AND `copyB.getIO() === io` (proves the slot survives duplication). Empirical post-fix Playwright re-verification: bob received `42["call:incoming",{...recipientDeptId:"cmpfmyva2000fa8k1drezs7bw"...}]` frame at tRel=7064ms (41ms after alice's click), bob.dialog.count=1. **Operational rule for future audits**: any module-local `let X = ...` that holds singleton state AND is reached via BOTH a dynamic-import chunk AND a static-import chunk in the Next.js build MUST hoist to `globalThis[Symbol.for(...)]`. The pattern matches Prisma's official Next.js singleton recipe (https://www.prisma.io/docs/orm/more/help-and-troubleshooting/help-articles/nextjs-prisma-client-dev-practices). **Latent risk sites in current codebase**: `createPresenceRoster()` and `createInCallRoster()` are passed to `attachPresenceHandlers`/`attachInCallHandlers` from inside `createSocketServer` so they live ONLY in the instrumentation-chunk copy and are never accessed cross-chunk — they're safe TODAY but would trigger the same trap if any tRPC route ever needed to call `roster.getOnlineUsers(orgId)` server-side. Audit-grep rule: `grep -rn "^let [a-zA-Z_]* *= *null" apps/web/src/server/` returns module-local singletons; verify each one is either (a) accessed only from one chunk OR (b) hoisted to globalThis. Cross-references: [[webpack-define-plugin-trap]] (sibling — DefinePlugin inlines NODE_ENV at build, this trap duplicates entire modules at build), [[turnstile-app-env-guard]] (third sibling — DefinePlugin trap remediation #3).

## 2026-05-23 — 🟡 [[turnstile-app-env-guard]] Third remediation site for the DefinePlugin trap — turnstile.ts:46 hostname-mismatch gate now keys on APP_ENV
- Type:      🟡 fix
- Phase:     Phase 7 follow-up SLOT 1 — closes action item from [[dev-compose-socket-port-exposure]] queued in 2026-05-23 PM handoff
- Files:     apps/web/src/server/lib/turnstile.ts (new pure helper `shouldEnforceTurnstileHostnameMatch({ APP_ENV })` + gate rewire), apps/web/src/server/lib/turnstile.test.ts (NEW, 3 RED→GREEN cases for dev/staging/prod)
- Concepts:  turnstile, cloudflare, siteverify, hostname-mismatch, define-plugin, app-env-vs-node-env, helper-extraction, tdd
- Narrative: Three remediation sites for the framework-level [[webpack-define-plugin-trap]] have now shipped: (1) `auth-bypass.ts` predicate `isE2EBypassEnabled` (commit ef17d8f), (2) `socket/auth.ts` cookie-name picker `isProduction` (commit 7787469), (3) this — `turnstile.ts:46` hostname-mismatch gate. Pre-fix the gate compared `env.NODE_ENV === "production"`; webpack's DefinePlugin inlined that as `"production" === "production"` at `next build` so the hostname check was always-on inside the containerized bundle. Cloudflare test sitekeys (`1x00000000000000000000AA` etc.) return `hostname: "localhost"` from siteverify — under the inlined gate, any test-key login attempt in containerized dev would compare "localhost" against e.g. `http://localhost:43512` → mismatch → 401 rejection. Fix: extracted the gate as pure helper `shouldEnforceTurnstileHostnameMatch({ APP_ENV })` matching the `isE2EBypassEnabled` shape — single responsibility, trivially unit-testable, returns `env.APP_ENV === "production"`. The call site passes `{ APP_ENV: env.APP_ENV }` so the runtime read survives bundling (env is a Zod-validated object, APP_ENV is read via process.env at boot, DefinePlugin only targets NODE_ENV). Bundle grep on chunk 9865.js post-fix confirms `APP_ENV:d._.APP_ENV}` and `APP_ENV?{success:!1` are PRESERVED as runtime property access — no string literal substitution. Tests: 3 RED→GREEN cases (dev=false, staging=false, prod=true); zero new gotchas because the trap is already locked at framework level. **Operational rule for future audits**: `grep -rn "env\.NODE_ENV" apps/web/src` should return zero matches at runtime comparison sites — only schema definition (env.ts:21) and getServerEnv pass-through (env.ts:78) are legitimate. Any other hit is a candidate trap. This grep should be part of any new-server-code review checklist. Cross-references: [[webpack-define-plugin-trap]] (framework rule), [[auth-bypass-prod-guard]] (first remediation), [[dev-compose-socket-port-exposure]] (second remediation + action item that filed this ticket).
...
- Files:     apps/web/src/env.ts, apps/web/src/server/auth-bypass.ts (was gating on NODE_ENV; switched to APP_ENV)
- Concepts:  webpack, define-plugin, nextjs, build-time-inlining, runtime-env, container-build, node-env, app-env
- Narrative: Next.js (via webpack's `DefinePlugin`) replaces every `process.env.NODE_ENV` reference in server-side code with the LITERAL build-time value during `next build`. So a containerized dev build that runs `next build` (which always uses `NODE_ENV=production` for the build itself, regardless of the target runtime) bakes `"production"` into the bundle. At runtime, even if the container env sets `NODE_ENV=development` via env_file, the bundled code STILL sees the inlined `"production"` constant. **Empirical proof from this incident**: searching the built chunks for `NODE_ENV:"production"` returned 3 matches (in chunks 3289.js, 7947.js, 984.js) hardcoded as string literal — the runtime env was `NODE_ENV=development` (verified via `docker exec yelli_dev_app sh -c 'echo $NODE_ENV'`) yet the bundle had the constant. Result: `isE2EBypassEnabled({ AUTH_BYPASS_FOR_E2E: true, NODE_ENV: "production" })` constant-folded to `false` during webpack tree-shaking, eliminating the bypass provider from `NextAuth({ providers: [...] })` entirely. `/api/auth/providers` returned only `{credentials}`, never `{e2e-bypass}`. **Verified that non-NODE_ENV env vars are NOT inlined**: searching the same chunks for `process.env.DATABASE_URL`, `process.env.AUTH_SECRET`, `process.env.REDIS_URL` returned 4 matches per var — all preserved as runtime reads. Only `NODE_ENV` is in DefinePlugin's static-replacement target (because Next.js + React rely on it for the dev/prod dispatch in framework code). **The fix**: switched the prod-guard from `NODE_ENV` to `APP_ENV` — a project-controlled env var added to `env.ts` serverSchema with the same `z.enum(["development", "staging", "production"])` validation. APP_ENV survives bundling as `process.env.APP_ENV` and is read at runtime, so the container env file controls the value. Defense-in-depth preserved: APP_ENV is set per environment by Phase 3 templates (.env.dev → "development", .env.staging → "staging", .env.prod → "production") and the dual-gate with AUTH_BYPASS_FOR_E2E still requires both flags to align. **Operational rule**: never gate runtime security logic on `env.NODE_ENV` in any code path that gets bundled by Next.js. NODE_ENV is for dev/prod dispatch within framework code (e.g. React DevTools warnings, console.error visibility) but is UNRELIABLE as a runtime env signal for app code. For environment-tier gating, always use a custom env var (APP_ENV is the established choice in this codebase). Cross-references: [[auth-bypass-prod-guard]] (the original design intent, now updated to use APP_ENV), [[playwright-smoke-auth-configuration-blocker]] (the original problem the bypass solves).

## 2026-05-23 — 🔴 [[auth-bypass-prod-guard]] AUTH_BYPASS_FOR_E2E must be dual-gated by APP_ENV !== "production" — single env-var gate is one accidental flag-flip away from prod auth bypass
- Type:      🔴 gotcha
- Phase:     Phase 7 (auth-bypass-for-e2e) shipped 2026-05-23; prod-guard switched NODE_ENV → APP_ENV in the (fix/auth-bypass-app-env-guard) hotfix later the same morning
...
- Concepts:  auth, e2e, playwright, security-guard, dual-gate, env-flag, prod-safety, app-env
- Narrative: The (auth-bypass-for-e2e) helper exists ONLY because Playwright smoke testing kept hitting the [[playwright-smoke-auth-configuration-blocker]] (Turnstile dummy tokens + stale Docker app container running an empty providers array). The implementation lets E2E sign in by email alone — no password, no Turnstile, no bcrypt. **A single env-var gate (`AUTH_BYPASS_FOR_E2E=true`) is NOT enough.** If that var ever leaks into a prod `.env` — by deploy-script error, by a copy-pasted staging config, by a developer setting it locally and pushing — every account in the platform is suddenly logged-in-able with just an email. The implementation enforces TWO independent conditions in `isE2EBypassEnabled()`: (1) `AUTH_BYPASS_FOR_E2E === true` AND (2) `APP_ENV !== "production"`. Both must hold. APP_ENV is set per environment by the Phase 3 env-file templates (.env.dev → "development", .env.staging → "staging", .env.prod → "production") and is project-controlled — distinct enough from AUTH_BYPASS_FOR_E2E that both would need to be misconfigured simultaneously to weaken auth. **WHY NOT NODE_ENV** (the original design): NODE_ENV is the more obvious "is this prod?" signal because Vercel/Komodo deploys lock it to "production" and `next build` sets it automatically. BUT webpack's DefinePlugin inlines `process.env.NODE_ENV` as a literal string constant at build time, so any runtime gate on env.NODE_ENV in a containerized Next.js production build constant-folds to false (or true, depending on the inlined value) regardless of container env. Verified empirically — see [[webpack-define-plugin-trap]] for the full forensic write-up. APP_ENV is NOT in DefinePlugin's target list and survives bundling as a runtime read. The predicate is extracted as a pure function and unit-tested across all three APP_ENV combinations + the false-flag case (auth-bypass.test.ts lines 53-87) so any future refactor that drops the prod guard will fail the test suite immediately. **Never collapse this to a single condition. Never gate the provider on just one env value. Never trust a single source of "is this prod?" truth.** Also: the provider is given a distinct id (`"e2e-bypass"`) — not the same id as the real Credentials provider — so Playwright must explicitly target it via `signIn("e2e-bypass", ...)`. Real prod-login UI calls `signIn("credentials", ...)` and never finds the bypass provider even if it somehow registered. Defense in depth at the provider-routing layer too.
