...
- §1 Tiered Decomposition — classify task complexity (Tier 1/2/3) with scoring formula
...
### Rule 24 — Fresh context per Phase 4 Part + STATE.md + Memory Governance (V14 + V31.1)
...
**Memory Governance integration (V31.1):** Rule 24's fresh-context principle is now reinforced by the Memory Governance Layer (`.claude/rules/memory-governance.md`). The Tiered Decomposition Engine (§1) runs at each Part's pre-flight to classify complexity. The Smart Checkpoint (§2) persists progress to Claude Code memory after each Part, enabling zero-cost resume. The Architect-Execute Model (§4) uses Opus 4.6 to plan Part decomposition and dispatch Sonnet 4.6 subagents for execution. This extends Rule 24 from "fresh context per Part" to "intelligently scoped context per task."
